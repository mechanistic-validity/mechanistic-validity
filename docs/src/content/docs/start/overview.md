...
The validity-type pages explain what each type asks of a claim and where current MI practice falls short. The casebook pages give the operational criteria, the metrics that produce evidence for each criterion, the failure modes that appear in practice, and a minimum reporting protocol.
...
The framework also does not assume any particular discovery method is correct. Activation patching, EAP, IIA, weight classifiers, and causal scrubbing all appear in the casebook pages as metrics that produce evidence for one or more criteria. None is privileged. The framework's role is to specify what each metric actually establishes.
