...
description: "Tests whether different instruments measuring the same construct produce correlated results."
---
...
Given two instruments \( A \) and \( B \) that each assign importance scores \( a_i, b_i \) to circuit heads \( i = 1, \ldots, H \), convergent validity is the rank correlation:
