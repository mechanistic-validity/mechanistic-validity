...
:::note
For the full metrics and protocols reference, see [Economics -- Metrics & Protocols](/framework/lenses/supporting/economics-metrics).
:::
...
- [IOI Circuit](/framework/examples/examples/examples-ioi) -- the economics lens can be applied via WC_M11 (Kyle Lambda); the existence of backup name-movers is a natural arbitrage test
- [Induction Heads](/framework/examples/examples/examples-induction-heads) -- signal discrimination is naturally high (induction heads respond specifically to repeated sequences, not random tokens)
- [Greater-Than](/framework/examples/examples/examples-greater-than) -- the successor heads' response to year-magnitude perturbations is a natural price impact curve
