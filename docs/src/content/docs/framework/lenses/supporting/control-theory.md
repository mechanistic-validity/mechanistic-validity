...
:::note
For the full metrics and protocols reference, see [Control Theory -- Metrics & Protocols](/framework/lenses/supporting/control-theory-metrics).
:::
...
- [IOI](/framework/examples/examples/examples-ioi) — the IOI circuit can be evaluated with this lens via WC_M1 (PID Steering); step response analysis across the 26-head circuit reveals settling behavior after name-mover ablation
- [Induction Heads](/framework/examples/examples/examples-induction-heads) — two-layer induction circuit provides a minimal test case for controllability (previous-token head as input) and observability (induction head output)
