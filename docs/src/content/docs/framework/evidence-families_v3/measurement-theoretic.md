...
description: "Meta-evidence evaluating whether instruments from other families produce trustworthy, stable, and meaningful measurements"
---

# Measurement-Theoretic Evidence

Measurement-theoretic evidence captures whether the measurements produced by other evidence families are trustworthy — evaluating instrument reliability, stability, and validity rather than model properties directly.

## What this family measures

Measurement-theoretic evidence is meta-evidential: it evaluates the quality of evidence from all other families rather than producing first-order claims about circuits. These instruments ask: "If I run this analysis again with different random seeds, do I get the same answer?" "If I use two different methods that should measure the same thing, do they agree?" "Can this instrument distinguish a real circuit from a baseline?"
...
- **F03 Convergent Validity** — Agreement between different instruments that should measure the same construct
- **F04 Discriminant Validity** — Disagreement between instruments that should measure different constructs
...
Measurement-theoretic evidence cannot establish that a circuit claim is true. It can only establish that the instruments used to evaluate the claim are (or are not) reliable. A perfectly reliable instrument can still be measuring the wrong thing — high test-retest reliability on a flawed metric is not progress.
...
- **M3 Baseline separation** — Whether instruments produce different scores for real circuits vs. random baselines
- **M4 Sensitivity** — Whether instruments can detect known differences (e.g., between a correct circuit and a slightly perturbed one)
- **M5 Calibration** — Whether confidence levels from instruments correspond to actual accuracy
- **M6 Construct coverage** — Whether the set of instruments used covers all relevant aspects of the claim being evaluated
