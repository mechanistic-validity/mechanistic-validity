...
O-information provides the group-level summary that [C04 (PID)](/framework/metrics/information/c04-pid/) details pairwise. A synergy-dominated circuit (\( \Omega < 0 \)) should show superadditive knockout effects in the [causal pillar](/framework/metrics/causal/) — removing any single head should degrade performance more than its individual contribution predicts. The [structural pillar](/framework/metrics/structural/) measures circuit connectivity, while O-information reveals whether that connectivity carries redundant or synergistic function.
