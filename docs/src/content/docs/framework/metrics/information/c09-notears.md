...
NOTEARS provides the structural graph that [C07 (Granger Causality)](/framework/metrics/information/c07-granger-causality/) and [C08 (OCSE)](/framework/metrics/information/c08-ocse/) estimate edge-by-edge. Where those methods test individual edges, NOTEARS jointly optimizes the entire graph with a global acyclicity constraint. The learned DAG can be compared against circuits recovered by the [causal pillar](/framework/metrics/causal/) (intervention-based) and the [structural pillar](/framework/metrics/structural/) (weight-based) — agreement across all three provides the strongest evidence for circuit validity.
