...
MI provides the foundation that [C04 (PID)](/framework/metrics/information/c04-pid/) decomposes into finer atoms. When MI is high but [C08 (OCSE)](/framework/metrics/information/c08-ocse/) scores are low, the component carries information but removing it does not degrade performance — suggesting redundancy. The [causal](/framework/metrics/causal/) pillar tests whether high-MI components are actually necessary via intervention.
