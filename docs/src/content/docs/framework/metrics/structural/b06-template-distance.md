...
This metric operates at the graph level rather than the weight level: it compares circuit topologies (which heads, which edges) rather than individual weight matrices. It is the structural complement to behavioral generalization tests — template distance asks whether the *structure* generalizes, not just the *performance*.
