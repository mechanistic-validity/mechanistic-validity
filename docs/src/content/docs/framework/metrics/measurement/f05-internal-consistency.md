...
description: "Assesses whether sub-parts of an evaluation metric produce coherent scores."
---

# F05 — Internal Consistency

This framework asks: **Do the individual items within our evaluation metric measure a single coherent construct?**

A circuit faithfulness evaluation typically aggregates scores across multiple prompts, multiple metrics, or multiple ablation targets. Internal consistency asks whether these sub-measurements hang together. If half the prompts suggest the circuit is faithful and the other half suggest it is not, the aggregate score is meaningless — the metric lacks coherence.
...
**What it does not establish:** That the construct being measured is the *right* one — only that the metric is internally coherent.
