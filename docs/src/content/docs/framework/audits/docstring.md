...
|---|---|---|
| **Primary.** Eight named heads compose to predict the next argument name in a Python docstring in this 4-layer model | Causally Suggestive | Specificity on any non-docstring behavior (I4), a leave-one-out over the eight heads (I3 Minimality), a matched random head set to show the 42% belongs to these heads (M2 Baseline separation), and dispersion on any reported number (M1 Reliability) |
| The circuit is the model's implementation of Docstring Induction, an algorithm | Underdetermined | Apportionment between induction and the line-number algorithm, which the authors judge co-implemented and suppress by prompt design rather than refute (I5 Rival mechanism exclusion, V3 Alternative level); the algorithm is inferred from attention patterns rather than tested against its rivals (V2 Level-evidence match) |
| The circuit behaves like the model on the task it was found on | Disconfirmed | Nothing — tested and failed (E2 Prompt generalization), and on post-origin evidence alone; the post's own remark that Google-style docstrings give similar performance points the other way and carries no number (M3 Stability) |
| These heads, or the head types they instantiate, serve other tasks or recur in other models | Insufficient | Cross-task reuse is never asked (E3 Cross-task generalization), cross-model transfer is named as future work with no model, head set or number reported (E4 Cross-model recurrence), and the circuit is never compared with one for a neighboring task (C4 Discriminant validity) |
...
| Mechanistically Supported | internal (I2 Sufficiency, I4 Specificity); external (E1 Intervention reach) | Blocked. **I4** specificity — Measured on one behavior; head 1.4 alone is run off-distribution |
