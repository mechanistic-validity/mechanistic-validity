...
**Circuit-only forward pass (complement [ablation](/mechanistic-validity/glossary/#ablation)):** Ablate all components *not* in the proposed circuit. Measure whether the circuit alone produces the target behavior at ≥ 70% of the full-model level (threshold should be pre-stated per C1 Falsifiability).
