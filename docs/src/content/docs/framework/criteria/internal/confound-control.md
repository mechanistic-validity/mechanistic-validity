...
I7 Confound control asks whether the causal effect is due to the nominated component's *specific computation* or to *collateral disruption*. It does not ask whether a *different* set of components could produce the same effect without collateral disruption — that is the domain of [I5 Rival Mechanism Exclusion](/mechanistic-validity/framework/criteria/internal/rival-mechanism-exclusion/). A claim can pass I7 (the effect is genuinely due to this component, not collateral damage) while failing I5 (other components could produce the same effect).
