...
Three of Hill's (1965) nine viewpoints for causal inference in epidemiology reappear as criteria: specificity (I4), convergence (addressed at construct level as C3 Convergent validity), and graded response (addressed at external level as E5). Mayo (1996, 2018) contributes the error-statistical tradition and the concept of severe testing — a test that a hypothesis was not built to pass. From genetics, we take epistasis for non-additive component interactions (I9 Epistatic interaction), rescue experiments for reversibility (I10), and sensitivity analysis for unmeasured confounders (I8 Confounding sensitivity). From medical microbiology, Koch's postulates provide the template for graded causal evidence: a pathogen must be found in all cases of the disease, isolated and grown in pure culture, and reproduce the disease when introduced into a healthy host. The parallel to circuit claims is direct — necessity, isolation sufficiency, and rescue — though the analogy is partial because circuits, unlike pathogens, are not discrete entities with sharp boundaries.
...
- **I7–I8 (confounders):** I7 addresses measured confounds (off-manifold ablation, backup suppression, layer-norm redistribution). I8 Confounding sensitivity asks how strong an unmeasured confounder would need to be to explain the result — the analog of Rosenbaum's sensitivity analysis in observational studies.
...
**IOI circuit specificity (I4).** Wang et al. (2022) discovered the IOI circuit on a specific set of prompts. Wu et al. (2026) test transfer within the task, across prompt templates: per-sample circuits of 712 and 754 edges share a Jaccard index of 0.126, and patching one sample with another sample's circuit moves the probability difference from $+0.120$ to $-0.224$. The circuit is specific to the prompt sample, not to the task.
