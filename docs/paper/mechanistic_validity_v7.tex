...
We audit fifteen mechanistic claims---the field's thirteen most-studied results, spanning circuits, a sparse-autoencoder feature, and probing classifiers, plus a steering vector and Anthropic's recent \emph{global workspace} result---auditing each \emph{claim} against the evidence base as it now stands rather than indicting any single paper. The framework discriminates: the indirect-object-identification circuit reaches Mechanistically Supported but caps there because cross-task specificity is untested; induction heads reach Triangulated; the gender-bias circuit is Disconfirmed, its construct too entangled with grammatical-gender knowledge for any causal evidence to separate. The same criteria place the global-workspace claim at Mechanistically Supported while its narrower deception-detection sub-claim rises closer to Triangulated---one paper, two tiers. We then turn the framework on itself, calibrating its verdicts against a synthetic circuit with known ground truth and a pre-registered prospective audit. These are findings about the field's evidence base, most produced without new experiments, by design.
\end{abstract}
...
\vspace{-12pt}
\caption{The Mechanistic Validity framework. Seven layers (7 modes, 4 families $\times$ 2 modes, 65 metrics, 30 criteria, 5 types, 9 synthesis protocols, 7 tiers), each answering a distinct question about the claim.}
\label{fig:pipeline}
\end{figure}
...
The Mechanistic Validity framework evaluates mechanistic interpretability claims through 30 criteria organized across five validity types in dependency order. Each claim receives a structured validity profile that makes gaps visible and the path to strengthening them concrete.
