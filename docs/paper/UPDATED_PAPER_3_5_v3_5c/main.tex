...
\citet{karvonen2025saebench} audit six standard SAE evaluation metrics and find that two fail basic diagnostics: scores on shuffled or random baselines are indistinguishable from scores on real features. A high score does not distinguish genuine feature quality from chance.

\paragraph{Vacuous nonlinear IIA (M2: baseline separation).}
\citet{sutter2025nonlinear} show that unconstrained nonlinear alignment maps achieve 100\% interchange-intervention accuracy on randomly initialized models. The metric measures the alignment map's flexibility, not the model's representational structure.

\paragraph{IOI circuit specificity (I3: specificity).}
The IOI circuit demonstrates necessity and sufficiency, but the original paper does not report whether ablating the circuit degrades performance on unrelated tasks. If it also degrades subject-verb agreement, what was discovered is a general-purpose subnetwork, not an IOI-specific mechanism.

\paragraph{Greater-than circuit negative controls (I3: specificity / C1: falsifiability).}
The greater-than circuit \citep{hanna2023greater} reports no negative controls: ablation effects on non-target tasks are never measured.

\paragraph{Knowledge neuron off-target effects (I3: specificity).}
\citet{dai2022knowledge} show that editing one factual association corrupts related facts. The intervention is not specific to the target knowledge.

\paragraph{Circuit non-uniqueness (I6: rival mechanism exclusion).}
\citet{meloux2025everything} show that activation patching returns alternative head sets with comparable faithfulness scores for the same task. The ``IOI circuit'' is \emph{one of several} sufficient subnetworks, not \emph{the} mechanism. This is Quine's underdetermination problem \citep{quine1951two} made concrete in mechanistic interpretability.

\paragraph{Double dissociation (I4: double dissociation).}
Almost never tested in MI. A proper double dissociation requires showing that ablating circuit A breaks task X but not Y, \emph{and} ablating circuit B breaks Y but not X. No published MI paper reports this test.

\paragraph{Method-conditional faithfulness (E1: intervention reach).}
\citet{miller2024transformer} show that the IOI circuit's 87\% faithfulness is specific to mean ablation; under resample ablation it drops below 50\%.

\paragraph{Cross-model replication (E6: cross-architecture generalization).}
The IOI circuit has been studied extensively in GPT-2 Small but has not been replicated in a second model family under pre-registered protocols.

\paragraph{Gender bias circuits (C4: discriminant validity).}
\citet{vig2020gender}: the heads mediating gender bias are the same heads encoding grammatical gender agreement. Ablating them reduces biased completions but equally degrades correct pronoun resolution. The construct presupposes a separation that does not exist.

\paragraph{Othello ``world model'' (V4: interpretive inflation).}
\citet{li2023othello} show that board state is linearly decodable from activations. The evidence supports ``decodable''; the label ``world model'' implies a computational-level claim about internal representation and planning.
...
\vspace{-12pt}
\caption{The \mechval{} pipeline. Seven layers, each answering a distinct question about the claim. The number of elements at each layer (7 modes, 4 sources, 84 metrics, 30 criteria, 5 types, 9 synthesis methods, 7 tiers) is not a design choice---it is the minimum needed to cover the failure modes observed in practice across mechanistic sciences. Collapsing any layer loses a distinction that has produced false conclusions in published work.}
\label{fig:pipeline}
\end{figure}
