...
\IfFileExists{tmlr.sty}{%
  ...
  \affil[1]{\texttt{elliot@elliottower.ai}}
  \date{}
}
...
Recent work provides uncomfortable answers. \citet{paulo2025sparse} demonstrate that only approximately 30\% of sparse autoencoder features are shared across independently trained SAEs on Llama~3 8B, rising to approximately 42\% on smaller models. \citet{gerasimov2026unstable} find that approximately 52\% of features are stable across 96 independently trained SAEs on GPT-2, with unstable features concentrating in reproducible lower-rank subspaces---suggesting basis ambiguity rather than meaninglessness. The majority of SAE features identified in any single training run may not replicate, raising serious concerns about the reliability of any safety-critical feature identification.

The field's primary causal tool fares no better. \citet{rezaeijafari2025relp} show that vanilla attribution patching---the most widely used method for identifying causally relevant components---correlates at $r = 0.006$ with ground truth for MLP outputs in GPT-2 Large on the IOI task. The correlation varies substantially by model size ($r = 0.195$ for GPT-2 Small, $r = 0.551$ for GPT-2 Medium), but the finding that performance degrades to essentially random rankings in larger models is troubling given that larger models are the ones that matter for safety applications.

The evaluation metrics used to assess these tools are themselves unreliable. The SAEBench audit \citep{chanin2026saebench} subjects six standard SAE evaluation metrics to three diagnostic tests: reseed stability, ground-truth correlation, and discriminability. Two of six---Targeted Probe Perturbation (TPP) and Spurious Correlation Removal (SCR)---fail multiple basic sanity checks. TPP exhibits reseed coefficients of variation between 16\% and 39\% at small top-$N$ settings.

Finally, the research itself does not reproduce. \citet{bai2026mecheval} perform execution-grounded evaluation of published MI research: over 90\% of papers fail reproducibility checks when their code is actually run, and 80\% fail coherence checks between claims and evidence. Narrative review---the kind performed by peer reviewers---missed 51 issues that execution-grounded evaluation caught.

The implication is direct: any claim of the form ``we found a feature that detects deception'' is built on this foundation. If the feature was identified using SAE methods, it may not replicate across training runs. If it was validated using attribution patching, the validation may be unreliable in larger models. If its quality was assessed using TPP or SCR, the assessment is unreliable. If the result was reported in a paper, the code likely does not reproduce.

This paper presents a framework for addressing this crisis. The Mechanistic Validity framework does not propose new detection methods. It provides a systematic protocol for determining whether any detection method---existing or future---actually works. The framework draws on established validation methodology from five fields (philosophy of science, neuroscience, psychometrics, pharmacology, and causal inference) to define 27 operational criteria, organized across five validity types in dependency order, with five verdict tiers that represent qualitative transitions in evidential status.
...
The Mechanistic Validity framework has five interconnected components: validity types (\Cref{sec:validity-types}), criteria (\Cref{sec:criteria}), evidence families (\Cref{sec:evidence-families}), verdict tiers (\Cref{sec:verdict-tiers}), and description modes (\Cref{sec:description-modes}).
...
Metrics are organized into five evidence families, each providing a different kind of evidence about a mechanistic claim. Each metric belongs to exactly one family. A sixth category---measurement calibrations---operates at a meta-level, assessing whether the primary metrics are themselves trustworthy.
...
The 14 measurement calibrations (F01--F15) are treated separately from the 84 primary metrics because they assess measurement quality rather than providing direct evidence about a mechanistic claim.
...
\centering
\tighttable
\caption{Current scale of the Mechanistic Validity implementation.}
\label{tab:scale}
\small
\begin{tabular}{@{}lr@{}}
\toprule
...
The 14 calibrations operationalize measurement validity: bootstrap stability (F01), seed variance (F02), convergent validity (F03), discriminant validity (F04), internal consistency (F05), split-half and test-retest reliability (F06), measurement invariance (F07), distributional characterization (F08), distributional stability (F09), nomological validity (F10), incremental validity (F11), ablation invariance (F12), method invariance (F13), and certified stability (F14). Three are hard gates: F01 (bootstrap), F08 (random baseline), and F09 (untrained baseline) must pass before downstream results are meaningful.

The 12 claim specifications cover circuits of varying complexity: IOI \citep{wang2023interpretability} (6 steps, 15 heads, 7 predictions, 5 negative controls), Greater-Than \citep{hanna2023greater} (3 steps, 7 heads), Induction \citep{olsson2022context} (2 steps, 7 heads), SVA \citep{lazo2025sva} (4 steps, 12 heads), Copy Suppression \citep{mcdougall2023copy} (3 steps, 7 heads), Gendered Pronoun (3 steps, 5 heads), Acronym \citep{garcia2024acronym} (3 steps, 8 heads), RTI (4 steps, 15 heads), and four epistemic framing variants discovered via different methods (manual, activation patching, EAP, and broad scan).
...
\midrule
\multirow{2}{*}{Triangulated} & Induction Heads & Simple mechanism, broad replication, thick nomological network \\
& IOI Circuit & 5+ independent groups, 4+ methods, cross-model validation (GPT-2, Pythia, Qwen, Gemma, Llama) \\
\midrule
\multirow{2}{*}{\parbox{2.3cm}{Mechanistically\\Supported}} & Greater-Than & Strong structural plausibility (C2); limited prompt generalization \\
...
\paragraph{IOI Circuit (Triangulated).} \citet{wang2023interpretability} identify 26 attention heads forming a six-role circuit for indirect object identification. The circuit demonstrates strong necessity (I1) and sufficiency (I2): running the model with everything outside the circuit mean-ablated recovers 87\% of the full model's logit difference. \citet{miller2024transformer} show that this figure is method-conditional (below 50\% under some ablation methods), but subsequent work has validated the circuit through multiple independent methodological families: ACDC automated discovery, edge attribution patching, contextual decomposition, and the MIB benchmark \citep{mueller2025mib}. Five or more independent research groups have confirmed the circuit across GPT-2, Pythia (70M--12B), Qwen, Gemma-2, and Llama-3. The circuit reaches Triangulated because it now satisfies criteria across independent evidence families (C3), has been replicated across model scales and architectures (E4), and has survived adversarial scrutiny. The remaining limitation is specificity (I3): no formal double-dissociation has been conducted, but the breadth of independent validation places IOI alongside Induction Heads as the field's best-validated circuit claim.
...
\paragraph{Method-conditional results.} The IOI circuit's original headline numbers were specific to mean ablation---but subsequent validation through multiple independent methods (ACDC, EAP, contextual decomposition) resolved this concern. Not all circuits are so lucky: ablation type is part of the causal claim, not an implementation detail. The framework captures this through E1 (intervention reach), which requires agreement across ablation methods.

\paragraph{The toy-model ceiling.} Grokking and Superposition achieve high validity within toy scope---indeed, Grokking's Fourier mechanism has been independently confirmed by multiple groups and could arguably reach Mechanistically Supported within its declared domain---but the gap between toy-model proof-of-concept and real-model confirmation remains the field's central challenge. The framework handles this through the scope declaration criterion (V5): a claim Validated within a declared scope is a legitimate result, not a failure.
...
The Mechanistic Interpretability Benchmark (MIB; \citealp{mueller2025mib}) provides competitive benchmarks for circuit discovery. Mechanistic Validity is complementary: MIB evaluates how well a method discovers circuits (Track~1); Mechanistic Validity evaluates how well-validated the discovered circuit is (Track~3). A circuit that achieves high CMD on MIB may still be poorly validated if it lacks sufficiency evidence, dose-response testing, or cross-method consistency. The two frameworks can be composed: discover a circuit via MIB-style methods, then evaluate it via Mechanistic Validity's validity criteria.

SAEBench evaluates SAE quality via standardized metrics. Mechanistic Validity's measurement validity criteria (M1--M6) and calibration gates (F01--F15) address a prior question: are the SAEBench metrics themselves trustworthy? The SAEBench audit's finding that TPP and SCR fail basic diagnostics is precisely the kind of measurement validity failure that the framework's gates are designed to catch.
...
The Mechanistic Validity framework addresses this gap by providing the measurement infrastructure that the field currently lacks. Its contribution is not a new detection method but a systematic protocol for determining whether any detection method actually works. The 27 criteria, five validity types, five evidence families, and five verdict tiers provide a common language for evaluating MI claims---whether they concern circuits, SAE features, probes, or steering vectors.
...
Bai, X., Baumgartner, A., Sun, H., Holtzman, A., and Tan, C.
\newblock The story is not the science: Execution-grounded evaluation of mechanistic interpretability research.
\newblock \emph{arXiv preprint arXiv:2602.18458}, 2026.
...
\bibitem[Chanin(2026)]{chanin2026saebench}
Chanin, D.
\newblock {SAEBench}: A diagnostic audit of six standard {SAE} evaluation metrics.
\newblock \emph{arXiv preprint arXiv:2605.18229}, 2026.
...
\bibitem[Gerasimov et~al.(2026)]{gerasimov2026unstable}
Gerasimov, A., et~al.
\newblock Unstable features, reproducible subspaces: 96 independently trained {SAEs} on {GPT}-2.
\newblock \emph{arXiv preprint arXiv:2606.12138}, 2026.
...
Kr{\'a}mar, J., Lieberum, T., Shah, R., and Nanda, N.
...
\bibitem[Paulo and Belrose(2025)]{paulo2025sparse}
Paulo, G. and Belrose, N.
\newblock Sparse autoencoders trained on the same data learn different features.
\newblock \emph{arXiv preprint arXiv:2501.16615}, 2025.
...
\bibitem[Rezaei Jafari et~al.(2025)]{rezaeijafari2025relp}
Rezaei Jafari, F., Eberle, O., Khakzar, A., and Nanda, N.
\newblock {RelP}: Faithful and efficient circuit discovery in language models via relevance patching.
\newblock In \emph{NeurIPS MechInterp Workshop}, 2025.
