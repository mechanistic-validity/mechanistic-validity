...
We audit 40 foundational MI papers (2021--2026) spanning circuit discovery, causal intervention, probing, and sparse autoencoders, counting the implicit comparisons behind each reported result. We count two types: \emph{pairwise} comparisons across results tables ($\binom{R}{2} \times C$ for a table with $R$ rows and $C$ metric columns); and \emph{search-space} comparisons, the candidate pool a finding was selected from---dictionary sizes, layer sweeps, component evaluations. None of the 40 report any multiple-comparison correction (Bonferroni, Benjamini-Hochberg, FDR, or permutation). The median implicit comparison count is 1,033, ranging from 0 (a pure resource release) to 223,977 (a vision-model decomposition scoring 3,498 text descriptions against 64 attention heads). One paper applies a $p < 0.01$ threshold to 10 syntactic relations independently---the uncorrected multiple testing Bonferroni was designed to prevent.

The correction is missing because of the venue, not the method. The same sparse-autoencoder technique gets a hypergeometric test with Benjamini-Hochberg correction when applied to protein language models in \emph{PNAS}, and Fisher's exact test with BH-corrected FDR in single-cell genomics; applied to GPT-2 for a NeurIPS or arXiv paper, it gets none. The statistical discipline is a property of the reviewing community, not of the analysis. Full audit in \Cref{appendix:comparison-audit}.
...
\citet{karvonen2025saebench} audit eight standard SAE evaluation metrics and find that two fail basic diagnostics: scores on shuffled or random baselines are indistinguishable from scores on real features. The metrics are reliable---they give the same answer when rerun---but a high score does not distinguish genuine feature quality from chance. Without a null distribution, there is no way to know whether an SAE feature score reflects real structure or noise that the metric cannot filter.
...
The 31 criteria also formalize common failure modes observed in published MI research (\Cref{tab:failures}).
...
\midrule
No baseline control & Report high score without testing a random or untrained model & M2 Baseline\newline separation & SAEBench: 2 of 8 SAE metrics indistinguishable from shuffled baselines \citep{karvonen2025saebench} \\
\midrule
...
\midrule
Off-target effects & Ablation works for the target task; other tasks never checked & I3 Specificity & Knowledge neurons: editing one fact corrupts related facts \citep{cohen2024evaluating} \\
\midrule
...
\vspace{-12pt}
\caption{The Mechanistic Validity framework. Seven layers (7 modes, 4 families $\times$ 2 modes, 65 metrics, 31 criteria, 5 types, 9 synthesis protocols, 7 tiers), each answering a distinct question about the claim.}
\label{fig:pipeline}
\end{figure}
...
\centering
\small
\caption{Thirty-one criteria across five validity types. Each criterion has a concrete test; a claim either passes, partially passes, or fails, producing a structured validity profile.}
\label{tab:criteria}
\renewcommand{\arraystretch}{1.15}
...
\midrule
Validated & All five validity types addressed & Measurement calibration (M1--M7) explicitly addressed; interpretive validity (V1--V5) audited; external validity across models (E4) and prompts (E2) \\
\midrule
...
Mechanistic Validity does not invent its criteria. It imports them from seven disciplines that each solved a different validity problem, applying them to interpretation as lenses: each contributes the conditions it requires before an inference is accepted, and names the failure mode it guards against (\Cref{tab:foundations-summary}). The load-bearing transfers appear where they do work rather than in a survey---confirmation versus corroboration in the severity rule (\Cref{sec:severity}), ablation versus patching as formally distinct interventions in the internal-validity criteria, dose-response and system reserve in the external-validity criteria, and description versus explanation in the interpretive criteria. The full per-discipline development, with sources and concept-by-concept mappings, is in \Cref{app:foundations}.

\begin{table}[h]
\centering\small
\caption{The seven foundations, each grounding a validity type. Full per-discipline treatment, with sources, in \Cref{app:foundations}.}
...
\midrule
Psychometrics & Measurement (M) & Reliability, baseline separation, and the convergent/discriminant pair from the multitrait-multimethod matrix \\
\midrule
...
We apply the framework to fifteen published mechanistic claims, evaluating each through all five validity lenses. We audit \emph{claims}, not papers: a verdict characterizes the current evidential status of a claim, aggregating all published evidence to date, and is not an assessment of any single paper's quality. Because verdicts are graded against an idealized standard, a low tier marks distance from that ideal, not a flawed study. Quantitative per-paper scoring is the subject of a companion paper; here we keep the analysis qualitative and trace each verdict to specific missing or present evidence.

\subsection{Validity Audit}

\Cref{tab:case-studies} presents the verdict assignments. The key finding is that the tier system produces meaningful discrimination: the fifteen claims span the full range from Proposed to Triangulated, with verdict differences traceable to specific missing evidence rather than arbitrary scoring.

\begin{table}[H]
\centering
\small
\caption{Verdict assignments for fifteen mechanistic claims, sorted by evidential status (see \Cref{tab:verdicts} for tier definitions).}
...
\midrule
Mechanistically Supported & Steering Vector (refusal direction) \citep{arditi2024refusal} & Necessity, sufficiency, and dose-response via direct intervention; caps on mode-match (a behavioral lever narrated as a mechanism, V2) and adversarial/scale fragility \\
\midrule
Mechanistically Supported & Global Workspace / J-space \citep{anthropic2026workspace} & Three converging interventions (swap, ablate, inject); caps below Triangulated on single evidence family, untested specificity (I3), and interpretive inflation (V4); deception sub-claim rises higher \\
\midrule
...
\midrule
Causally Suggestive & SAE Features \citep{cunningham2023sparse, leask2025canonical} & Not atomic or canonical (large-dictionary latents decompose into finer ones, C4); seed-dependent (M1/M3); no better than raw neurons at causal localization---features may be dictionary properties, not model properties \\
\midrule
...
This is a discriminant validity (C4) failure: the measure cannot distinguish the target construct from its neighbor. One might argue that the construct needs refinement rather than rejection---perhaps ``bias'' should be defined relative to corpus frequency rather than as an absolute property. But any such refinement must first demonstrate that the refined construct is separable from gender knowledge in the model's representations, which is a testable prediction the current evidence does not support. Unlike the other case studies, where the path forward is more evidence, the path forward here is reformulation of the question. The framework surfaces this distinction: a claim can fail not because the experiments were poorly done but because the construct was not well-posed. Five years on, this remains a gap rather than a settled refutation: no subsequent study has directly tested whether bias and grammatical-gender knowledge are separable in the representations, and disentangling grammatical from social gender is still treated as open. The Disconfirmed verdict rests on a construct that presupposes a separation the one relevant mediation analysis contradicts and that no work has since established---the framework's point about discriminant validity going untested, made concrete.

\paragraph{SAE Feature (Causally Suggestive).} A sparse-autoencoder feature labeled as a concept---``this feature is deception,'' ``this feature is the Golden Gate Bridge''---claims that the model represents that concept along a recovered direction. The evidence is usually observational: the feature activates on concept-related text, and its label is read off maximally-activating examples. That establishes decodability, not use, so absent an intervention the claim inherits the ceiling of the probing case. Two recent lines of evidence push it lower on construct and measurement validity. \citet{leask2025canonical} show SAE features are neither complete nor atomic: larger dictionaries recover latents that smaller ones miss, and a single ``Einstein'' latent decomposes into ``scientist,'' ``Germany,'' and ``famous person''---a discriminant-validity (C4) failure, since the labeled unit is a composite of neighboring concepts. Feature identity also shifts with the training seed (M1/M3), and on the Mechanistic Interpretability Benchmark features localize causal variables no better than raw neurons \citep{mueller2025mib}. A feature-clamping intervention can lift a specific feature to Causally Suggestive, but the generic claim that features are canonical units of analysis caps at Proposed.

\paragraph{Steering Vector (Mechanistically Supported).} A steering vector claims that adding a direction to the residual stream drives a target behavior. Unlike a feature or a probe, this is interventional by construction---a $\mathrm{do}$-operation on the activations---so it clears the causal bar the observational cases cannot, and it admits a dose-response by scaling the vector (E5). The refusal direction \citep{arditi2024refusal} is the strongest instance: ablating it removes refusal, adding it induces refusal, and the effect replicates across open-weight models. The claim caps on interpretive rather than internal validity. A direction that is a sufficient behavioral \emph{lever} is routinely narrated as \emph{the representation} the model uses for the concept---a description-mode mismatch (V2), the affinity-versus-efficacy confusion pharmacology names. The literature is itself drifting toward the weaker reading: chain-of-thought reasoning disrupts single-direction steering, adversarial optimization defeats it, and its effect degrades with scale---boundary conditions on E5 and E6 that a lever-not-mechanism account predicts and a representational account does not.

\paragraph{Global Workspace / J-space (Mechanistically Supported).} Anthropic's Jacobian lens recovers a low-dimensional residual-stream subspace---the ``J-space''---by averaging causal influence on the output logits across contexts, and narrates it as a ``global workspace'' holding the concepts the model reasons over \citep{anthropic2026workspace}. The mechanistic core is unusually strong: three converging interventions---swapping a concept in the subspace, ablating the subspace, injecting a concept---each move behavior as predicted, establishing necessity and sufficiency. The claim nonetheless caps at Mechanistically Supported, for reasons the framework makes precise. The evidence is one family (interventional activations); an independent replication on open-weight Qwen adds a model, not a method. Specificity (I3) is untested---if ablating the subspace degrades every task that coordinates many representations rather than deliberate reasoning alone, what was found is a general high-influence bottleneck, not a workspace: the untested-specificity structure of the cardiac-stent case. And the ``global workspace'' label is a computational claim resting on representational evidence---an interpretive inflation (V4) sharpened by a circularity, since the subspace is \emph{defined} as the directions most influential on the output, so its correlation with deliberate reasoning is guaranteed by construction. The paper's strongest result escapes these limits: its finding that the subspace carries deception-relevant signal before any deceptive output has an explicit contrastive foil (aligned versus deceptive internal state) and needs no workspace narrative, and that narrower claim rises closer to Triangulated. One paper thus houses two claims at two tiers---the reason the audit scores claims, not papers.
...
The audit in \Cref{sec:case-studies} scores published claims, but it invites an obvious objection: the criteria were designed by the same author who then applied them, so the verdicts could be retrofitted. A framework about validity has to survive being turned on itself. We do so three ways.

\paragraph{A ground-truth calibration.} The Validated tier is reachable only against a claim whose mechanism is known independently of the audit. Grokking on modular addition supplies one: a small transformer trained to generalization implements modular arithmetic through a Fourier construction whose weights are predictable in closed form, and a model learns \emph{either} a ``Clock'' or a ``Pizza'' algorithm, distinguishable by a precise Fourier-domain signature \citep{nanda2023progress}. This dual mechanism is a stronger calibration target than a single ground truth, because it tests \emph{discrimination}: running the full measurement (M1--M7) and interpretive (V1--V5) suites on a trained model, the framework should not only certify the true mechanism at Validated but deny that tier to a claim asserting the wrong one of the two. A framework that awards Validated to both, or to neither, is miscalibrated. This is the one place the framework must run experiments rather than audit existing evidence, and it is where the top tier earns its meaning.

\paragraph{A pre-registered prospective audit.} Retrospective scoring cannot rule out retrofitting on its own. We therefore pre-register verdicts on newly published claims before their evidence is fully reviewed, freezing the predicted tier and its rationale at a public commit hash, and report how the prospective verdicts land. A verdict committed before the evidence is a severe test of the framework in exactly the sense the framework demands of the claims it audits---the same discipline, applied reflexively.

\paragraph{Inter-rater reliability.} Because aggregating criterion-level results to a tier involves judgment, two evaluators could assign different tiers to the same claim. The framework mitigates this by making the structured validity profile, not the tier, the primary output: disagreement localizes to individual, independently falsifiable criterion judgments rather than a holistic impression. We report agreement between independent raters at the criterion level, where it matters, rather than only at the tier.
...
The Mechanistic Validity framework evaluates mechanistic interpretability claims through 31 criteria organized across five validity types in dependency order. Each claim receives a structured validity profile that makes gaps visible and the path to strengthening them concrete.
...
Full 31-criteria audits, evidence cards, and circuit specifications for the case studies evaluated in \Cref{sec:case-studies}.
...
We audit the IOI circuit claim \citep{wang2023interpretability} against all 31 criteria to demonstrate what the framework produces in practice. The claim: ``The IOI circuit uses 26 attention heads in 6 functional roles to identify and output the indirect object in sentences of the form `When Mary and John went to the store, John gave a drink to \underline{\hspace{1cm}}'\,.'' Description mode: implementational-functional.

\begin{table}[H]
\centering
\small
\caption{Full 31-criteria audit of the IOI circuit. Status: \textbf{C} = Confirmed, \textbf{PC} = Partially confirmed, \textbf{U} = Untested, \textbf{I} = Inconclusive, \textbf{D} = Disconfirmed.}
\label{tab:ioi-audit}
...
M7 & Selection correction & U & Implicit comparisons not reported; no multiplicity control \\
\midrule
...
\paragraph{Summary.} Of 31 criteria: 4 Confirmed, 9 Partially confirmed, 1 Inconclusive, 17 Untested, 0 Disconfirmed. The IOI circuit reaches \textbf{Mechanistically Supported} (necessity and sufficiency established via one method family). The bottleneck to Triangulated is the absence of convergent evidence from $\geq 3$ evidence families (C3), untested specificity (I3), and no cross-model replication (E4). The bottleneck to Validated is the mostly unaddressed measurement validity block (M1--M7; only M4 is partially confirmed).
...
% ============================================================
\section{Multiple Comparison Audit}
\label{appendix:comparison-audit}

We audit 40 foundational MI papers (2021--2026) for implicit comparison counts. For each paper we count pairwise comparisons (row comparisons across results tables: $\binom{R}{2} \times C$) and search-space comparisons (the candidate pool from which reported findings were selected). The top 10 papers by total implicit comparisons are shown in \Cref{tab:audit-top10}. None of the 40 papers report any multiple-comparison correction. Full per-paper audits are available in the supplementary materials.

\begin{table}[H]
\centering
\small
\caption{Top 10 MI papers by total implicit comparison count $m$. None apply any correction.}
\label{tab:audit-top10}
\begin{tabular}{@{}llrrr@{}}
\toprule
\textbf{Paper} & \textbf{Subfield} & \textbf{Pairwise} & \textbf{Search} & \textbf{Total $m$} \\
\midrule
Gandelsman et al.\ (2023) & Vision & 105 & 223{,}872 & 223{,}977 \\
Tang et al.\ (2022) & Vision & 543 & 21{,}324 & 21{,}867 \\
Zou et al.\ (2023) & Probing & 784 & $\sim$5{,}000 & $\sim$5{,}442 \\
Stolfo et al.\ (2023) & Circuit & 417 & 4{,}768 & 5{,}185 \\
Mueller et al.\ (2025) & Benchmark & 3{,}151 & $\sim$510 & 3{,}661 \\
Marks \& Tegmark (2023) & Probing & 3{,}267 & 152 & 3{,}419 \\
Rimsky et al.\ (2023) & Causal & 1{,}977 & 1{,}330 & 3{,}307 \\
Nanda et al.\ (2023) & Circuit & 1{,}983 & 572 & 2{,}555 \\
Wu et al.\ (2023) & Causal & 165 & 2{,}243 & 2{,}408 \\
Karvonen et al.\ (2025) & Benchmark & 0 & $\sim$2{,}320 & $\sim$2{,}320 \\
\bottomrule
\end{tabular}
\end{table}

% ============================================================
\section{Theoretical Foundations: Full Treatment}
