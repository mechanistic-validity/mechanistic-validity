...
\item \textbf{Verdict.} What tier does the claim reach? Mechanistically Supported---necessity and sufficiency are established with at least one method, but evidence comes primarily from one methodological family (ablation-based), key criteria remain untested (I3, I4, E4), and the faithfulness result is method-conditional.
\end{enumerate}
...
\vspace{-12pt}
\caption{The \mechval{} pipeline. Seven layers, each answering a distinct question about the claim. The number of elements at each layer (7 modes, 4 sources, 84 metrics, 30 criteria, 5 types, 9 synthesis protocols, 7 tiers) is not a design choice---it is the minimum needed to cover the failure modes observed in practice across mechanistic sciences. Collapsing any layer loses a distinction that has produced false conclusions in published work.}
\label{fig:pipeline}
\end{figure}
...
\midrule
Proposed & Construct defined under a declared description mode (C1--C2 satisfied); at least one admissible measurement conducted \\
\midrule
...
Each metric takes a model, a task, and optionally an intervention as input, and produces a number as output. Metrics can be composed: one metric's output feeds into another. The framework organizes evidence into four evidence families; the current catalog defines 84 metrics across families A--D, drawing on interventionist, information-theoretic, structural, and generalization traditions of causation. The full inventory is in \Cref{appendix:metrics}.
...
Synthesis methods aggregate evidence across multiple methods to evaluate convergent validity (C3) and external validity (E2--E4). When multiple discovery methods each identify a set of circuit components, the question is whether they agree---and how to handle the fact that methods have different reliability profiles. The framework defines nine synthesis protocols, each adapted from an established aggregation method in another field: Jaccard overlap as a naive baseline; Robust Rank Aggregation \citep{kolde2012robust} for ranked lists; Dawid-Skene consensus \citep{dawid1979maximum} for reliability-weighted voting; parallel and sequential ensembles for fusing or filtering across methods; Wasserstein distributional stability for cross-context generalization; functional parcellation (adapted from \citealp{glasser2016multi}) for identifying component subgroups; a meta-learner that predicts circuit membership from protocol features; Granger causality graphs for cross-family predictive structure; and modular circuit vocabulary \citep{he2025modular} for identifying reusable subgraph motifs across tasks. The detailed specifications are in \Cref{appendix:synthesis}.
...
% TODO: Update foundations-pyramid.pdf to reflect genetics + causal inference additions, or drop figure
\begin{figure}[H]
\centering
...
This appendix provides: a full 30-criteria audit of the IOI circuit (Appendix~\ref{appendix:ioi-audit}), an example circuit specification (Appendix~\ref{appendix:ioi-detail}), the evidence card reporting format (Appendix~\ref{appendix:evidence-cards}), the full metric inventory grouped by evidence family (Appendix~\ref{appendix:metrics}), protocol specifications (Appendix~\ref{appendix:protocols}), and synthesis protocol details (Appendix~\ref{appendix:synthesis}).
...
\paragraph{S01: Functional Parcellation.} Synthesizes component rankings across evidence families into functional subgroups. Adapted from \citet{glasser2016multi}'s multimodal brain parcellation. Takes ranked component lists from 4+ methods, computes representational similarity between the rankings, and clusters components that are consistently co-ranked. The output is a set of functional groups---components that multiple independent methods agree belong together. Strengthens C5 (convergent validity) and I3 (specificity).

\paragraph{S02: Dawid-Skene Consensus.} Synthesizes binary circuit membership judgments across methods with different reliability profiles. Treats each method as a noisy annotator and jointly estimates (via EM) both the true circuit membership and each method's reliability \citep{dawid1979maximum}. Methods that consistently disagree with the majority get downweighted. Strengthens C5 (convergent validity) and M1 (reliability).

\paragraph{S03: Robust Rank Aggregation.} Synthesizes ranked component lists into a single ranking with statistical significance. Computes RRA $p$-values and Borda counts across method rankings \citep{kolde2012robust}. Identifies components that rank consistently high across methods (robust members) and components that rank high in one method but low in others (method-dependent members). Strengthens C5 (convergent validity) and I5 (confound control).

\paragraph{S04: Parallel Ensemble.} Synthesizes rank-normalized scores across methods into a single composite ranking. Implements three fusion rules---equal weighting, method-reliability weighting, and minimum-across-methods---with uncertainty bounds. Strengthens C5 (convergent validity) and M3 (baseline separation).
...
\paragraph{S07: Meta-Learner.} Synthesizes method features into a predictive model of circuit membership. Trains a logistic regression on known-labeled circuits (components with established ground truth) using method features as predictors. The learned coefficients reveal which method features are most predictive of true membership. Strengthens M4 (sensitivity) and C5 (convergent validity).

\paragraph{S08: Granger Causality Graph.} Synthesizes cross-family predictive relationships between method scores. Constructs a directed graph where edges represent Granger-causal relationships---does component $A$'s score in method $X$ predict component $B$'s score in method $Y$, conditional on $B$'s own score in $X$? Cross-family predictive structure indicates genuine convergent support rather than shared noise. Strengthens C5 (convergent validity) and I4 (consistency).

\paragraph{S09: Modular Circuit Vocabulary.} Synthesizes circuit structure across tasks into reusable computational primitives. Adapted from \citet{he2025modular}. Identifies reusable circuit subgraphs shared across tasks---if the same three-head subgraph appears in the IOI circuit, the greater-than circuit, and the induction circuit, it is a building block rather than a task-specific artifact. Strengthens E6 (cross-architecture generalization) and C2 (structural plausibility).
