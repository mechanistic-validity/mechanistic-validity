...
\title{Mechanistic Validity: Scientific Formalization and Theory of Validity for Interpretability Claims}
...
% ============================================================
\begin{abstract}
A mechanistic explanation of a neural network is not a single experimental result, but a structured claim linking a theoretical construct, a measurement procedure, an intervention, and a scope of interpretation. We introduce Mechanistic Validity (\mechval{}), a formalized epistemology for validating these causal claims, drawing on validation methodology from causal inference, measurement theory, and the philosophy of science. The framework makes interpretability claims explicit, falsifiable, and comparable, formalizing the discovery, validation, and interpretation of causal mechanisms as separate evidential stages. A central problem the framework addresses is that post-hoc mechanistic narratives are unfalsifiable: given any set of ablation results, a plausible story can always be constructed to fit them. \mechval{} encodes mechanistic claims as explicit graphs with testable predictions and negative controls---specifying what the mechanism should \emph{not} affect---so that the absence of falsification tests is visible in the validity profile. \mechval{} organizes 27 operational criteria across five validity types---construct, measurement, internal, external, and interpretive---in dependency order, and assigns each claim to one of five verdict tiers from Proposed to Validated. It is method-agnostic, applying the same criteria to circuits, SAE features, probing classifiers, steering vectors, transcoders, and crosscoders, and is supported by an open-source implementation with 84 metrics across five evidence families, 14 calibration gates, and 12 pre-registered claim specifications. Claims are additionally qualified by one of Marr's \citeyearpar{marr1982vision} three canonical levels of description---computational, algorithmic, and implementational---which we extend with representational as a fourth level and with four implementational sub-types: topographic, connectomic, activation-statistical, and functional. Applying the framework to 13 published circuits, we show that the tier system discriminates among claims---from Proposed (knowledge neurons, probing classifiers) to Triangulated (induction heads)---with every verdict traceable to specific present or missing evidence. By making the evidential status of a claim explicit and the path to strengthening it concrete, \mechval{} aims to help interpretability mature into a discipline whose claims are both interpretable and scientifically validated.
\end{abstract}
...
The \mechval{} framework introduced below encodes mechanistic claims as explicit causal
graphs---with nodes for computational roles, edges for claimed dependencies, and non-edges
for claimed independences---each with testable predictions that can survive or fail
refutation attempts. Without such structure, a mechanistic narrative that was retrofitted
to match ablation results is indistinguishable from one that predicted them. This matters
because a mechanistic explanation has many adjustable parts---which components to include,
what roles to assign, which edges to draw---and a narrative with enough flexibility will
always fit the observations.
...
(\Cref{sec:foundations}), its implementation (\Cref{sec:implementation}), its application
to 13 published circuits (\Cref{sec:case-studies}), and its implications for the field
(\Cref{sec:discussion}).
...
Finally, the LLM-as-judge paradigm \citep{zheng2023judging} has been used for open-ended evaluation, and a companion paper (Tower, 2026, in prep.) automates \mechval{} via LLM extraction. Automation is treated separately because the framework's correctness is independent of whether scoring is done by hand or by machine.
...
This claim implicitly encodes a graph---not of the model's architecture or its causal abstraction, but of what the researchers are \emph{claiming} and what they are \emph{not claiming}. The claim says that S-inhibition heads feed name-mover heads (a claimed dependency). It also implicitly says that name-mover heads do \emph{not} feed back into S-inhibition heads (a claimed independence). These implicit non-edges---the things a claim takes for granted without testing---are rarely made explicit, and they are where most claims would fail if tested.

The \mechval{} framework makes this implicit graph explicit as a \emph{ClaimSpec graph}: a graph whose nodes are claimed computational roles, whose edges are claimed dependencies with testable positive predictions, and whose non-edges are claimed independences with testable negative controls. The graph represents the structure of the \emph{claim}, not the structure of the model.

Given a ClaimSpec graph, the framework evaluates it through six layers (\Cref{fig:pipeline}):
...
The ClaimSpec graph serves as an evidence organizer: as experiments accumulate, each result is linked to the specific edge or non-edge it tests, and the framework tracks which criteria are covered and which remain gaps. This is useful regardless of when the graph is written---during discovery, after discovery, or before experiments begin. Pre-registering the graph before running experiments is one way to use it, and the framework rewards this with a scoring bonus, because a pre-registered graph with negative controls on its non-edges can actually fail---unlike a narrative that was retrofitted to match observations, which can always redraw its edges to fit. But the primary value is organizational: making the structure of a claim and the state of its evidence explicit and trackable.

The ClaimSpec graph addresses two kinds of problems. First, it prevents \emph{process failures}---methodological errors that compromise evidence before it is even evaluated (\Cref{tab:process_failures}).

\begingroup
\small
\begin{table}[H]
\centering
\caption{Process failures addressed by ClaimSpec graphs.}
\label{tab:process_failures}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{5.5cm}@{}}
\toprule
\textbf{Process failure} & \textbf{What happens} & \textbf{How ClaimSpec helps} \\
\midrule
Cherry-picking metrics & Run multiple methods, report only the best number & ClaimSpec commits to which methods count and what threshold they must meet before results are seen \\
\midrule
Post-hoc role labeling & Name heads after observed behavior; the label cannot fail & ClaimSpec registers the label as a testable prediction: ``this head specifically moves names, not any attended token'' \\
\midrule
Circuit redefinition & Drop heads that hurt faithfulness, call it ``refined'' & ClaimSpec commits to circuit membership before measuring faithfulness; dropping heads requires a new spec \\
\midrule
No negative controls & Only test predictions that should pass & ClaimSpec requires non-edges: explicit predictions about what the mechanism should \emph{not} affect \\
\midrule
Flexible scope & Narrow the claim after seeing results so it cannot fail & ClaimSpec declares scope before evaluation; post-hoc narrowing is visible as a spec change \\
\bottomrule
\end{tabular}
\end{table}
\endgroup

Second, the 27 criteria detect \emph{evidential failures}---gaps in the evidence that weaken a claim even when the process was sound (\Cref{tab:failures}).

\begingroup
\small
\begin{table}[H]
\centering
\caption{Evidential failures detected by the 27 criteria.}
\label{tab:failures}
\renewcommand{\arraystretch}{1.15}
...
\midrule
No baseline control & Report high IIA without testing a random model & M2 Baseline\newline separation & Nonlinear maps achieve 95\% IIA on random models \citep{sutter2025nonlinear} \\
\midrule
Off-target effects & Ablation works for target task; other tasks never checked & I3 Specificity & Knowledge neurons: editing one fact corrupts related facts \\
\midrule
...
\midrule
Interpretive inflation & Label implies intent beyond what evidence shows & V4 Anthropo-\newline morphism check & ``World model'' for Othello (evidence: ``linearly decodable'') \\
\midrule
...
\midrule
Ignoring alternatives & Circuit works, but so does a different set of heads & I4 Double dissoc.\newline C3 Convergent val. & Meloux et al.\ find alternative IOI circuits \\
\midrule
Construct incoherence & Construct not separable from a neighboring one & C4 Discriminant\newline validity & Gender bias: bias and knowledge share the same heads \\
\bottomrule
\end{tabular}
\end{table}
\endgroup

The rest of this paper defines each layer precisely (\Cref{sec:framework}), grounds them in established methodology from five fields (\Cref{sec:foundations}), describes the implementation (\Cref{sec:implementation}), and applies the framework to 13 published circuits (\Cref{sec:case-studies}).
...
Metrics are runnable tests that produce measurements about a neural network. They form the empirical base of the framework: evidence families organize them, criteria evaluate them, and verdicts aggregate them. The current implementation includes 84 metrics across families A--E and 14 calibrations in family F (\Cref{sec:scale}). \Cref{tab:causal_metrics} illustrates the causal family; the full inventory for all six families is in \Cref{app:metrics}.
...
\Cref{tab:crossfield} shows how the scoring views map to established methods across the five source fields.
...
\caption{Cross-field mapping: scoring views mapped to disciplinary methods.}
\label{tab:crossfield}
\begin{tabular}{|p{2.2cm}|p{2.2cm}|p{2.2cm}|p{2.2cm}|p{2.2cm}|p{2.2cm}|}
\hline
\textbf{View} & \textbf{Causal Inference} & \textbf{Neuroscience} & \textbf{Psychometrics} & \textbf{Pharmacology} & \textbf{Phil. of Sci.} \\
\hline
Effect Estimation & ATE / CATE & Perturbation quant. & Effect size est. & Dose-response & Parameter estimation \\
\hline
Transportability & Transportability & Cross-condition gen. & Measurement invariance & Phase III generalization & Novel prediction \\
\hline
Counterfactual Verification & Rung-3 counterfactual & Optogenetics & --- & --- & Counterfactual \\
\hline
Mechanism Adjudication & Equiv. models & --- & Model comparison & --- & Crucial experiment \\
\hline
\end{tabular}
\end{table}
...
\paragraph{A worked example.} Consider the IOI claim that the three Name Mover heads (9.9, 9.6, 10.0) copy the indirect object token to the final position. The spec encodes one positive prediction---ablating the Name Mover role reduces logit difference by at least 0.2---and one negative control---ablating Name Movers does not disrupt upstream S-Inhibition heads (effect $< 0.05$), confirming that the roles are directionally connected (S-Inhibition feeds Name Movers) rather than bidirectionally entangled. When \texttt{verify(spec)} runs, role ablation drops logit difference by 0.34 (positive prediction passes) and shifts S-Inhibition by only 0.02 (well below the 0.05 threshold, so the negative control passes---Name Movers do not affect their upstream input). A failure on either side is informative: a missed positive prediction caps the claim ceiling at a lower description mode, while a missed negative control flags a specificity failure (I3).

\subsection{Scale}
\label{sec:scale}

The current implementation includes 84 metrics across the five primary evidence families: 32 causal (activation patching, DAS-IIA, mediation, CATE, sigma ablation, causal scrubbing, EAP, path patching), 18 structural (composition scores, template distance, graph topology, spectral analysis), 9 information-theoretic (PID, OCSE, NOTEARS, mutual information, transfer entropy), 20 behavioral (dose-response, faithfulness, generalization gap, boundary analysis), and 5 representational (CKA, probes, attention entropy). The 14 calibrations in family F operationalize measurement validity; three are hard gates (bootstrap stability, random baseline separation, untrained baseline separation) that must pass before downstream results are considered meaningful.

The framework includes 12 pre-registered claim specifications covering circuits of varying complexity---from Induction (2 steps, 7 heads) to IOI (6 steps, 15 heads, 7 positive predictions, 5 negative controls)---and 13 worked case studies evaluated through all five validity lenses.
...
\section{ClaimSpec Graph Examples}
\label{app:claimspec_examples}

\Cref{fig:claimspec_knowledge} shows the ClaimSpec graph for knowledge neurons: a single node with one positive prediction (ablating the neuron degrades recall of a specific fact) and four untested non-edges. Without testing the non-edges, ``knowledge neuron'' is indistinguishable from ``middle-layer neuron that disrupts everything when ablated.''

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{fig_claimspec_knowledge_v2.png}
\caption{ClaimSpec graph for knowledge neurons. The positive prediction (green arrow) passes, but the non-edges (orange dashed arrows) are untested---leaving specificity unestablished.}
\label{fig:claimspec_knowledge}
\end{figure}

\Cref{fig:claimspec_gender} shows gender bias circuits: the claim requires that ``gender bias'' and ``gender knowledge'' be separable constructs. But both map to the same attention heads. The required non-edge does not exist---you cannot ablate bias without ablating knowledge. The claim graph collapses, revealing construct incoherence.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{fig_claimspec_gender_v2.png}
\caption{ClaimSpec graph for gender bias circuits. The claim requires separability between bias and knowledge (red dashed non-edge), but both map to the same heads---a construct-level failure.}
\label{fig:claimspec_gender}
\end{figure}

\section{Example Claim Specification}
\label{app:ioi-spec}
