...
Metrics are organized into five primary evidence families plus a sixth measurement-quality family. The 84 metrics belong to families A--E; the 14 calibrations in family F assess the trustworthiness of the primary metrics rather than providing direct evidence about the claim.

\paragraph{A. Causal.} Effects of interventions---ablation, patching, clamping, resampling. Examples: activation patching, DAS-IIA, CATE, mediation analysis, sigma ablation.

\paragraph{B. Structural.} Weight-space properties, requiring no forward pass. Examples: SVD, effective rank, OV/QK composition norms, template distance.

\paragraph{C. Information-theoretic.} Information flow and dependencies. Examples: mutual information, partial information decomposition, OCSE, transfer entropy.

\paragraph{D. Behavioral.} Input-output behavior under manipulation. Examples: faithfulness, logit difference, KL divergence, dose-response curves.

\paragraph{E. Representational.} Geometry and content of internal representations. Examples: CKA, RSA, linear probes, attention entropy.

\paragraph{F. Measurement.} Meta-properties of other metrics---calibrations that assess whether the primary metrics are trustworthy. Examples: bootstrap stability, seed variance, convergent validity, discriminant validity.
...
A metric is a concrete, runnable test that produces a measurement about a neural network---the layer of the framework where empirical contact with the model actually happens. The current implementation includes 84 metrics across the five primary evidence families and 14 calibrations in family F (\Cref{sec:scale}). The full metric inventory is listed by family in \Cref{app:metrics}.
...
\hline
\textbf{ID} & \textbf{Criterion and description} \\
\hline
