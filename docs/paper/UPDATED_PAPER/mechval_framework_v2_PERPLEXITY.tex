...
% TODO(figure, optional): a vertical tier ladder graphic showing the five
% verdict tiers stacked, with each rung labeled by its required new evidence
% (I1 -> I1+I2 -> +E1 -> +convergence -> +M+V). Skipped in v2 because the
% requirements column of Table 2 already conveys the structure.
...
% Transposed cross-field mapping (columns/rows swapped for readability)
\begin{table}[t]
\centering
\small
\renewcommand{\arraystretch}{1.1}
\caption{Cross-field mapping (transposed): methods/components across scoring tracks and views.}
\label{tab:crossfield_transposed}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{|l|p{1.6cm}|p{1.6cm}|p{1.6cm}|p{1.6cm}|p{1.6cm}|p{1.6cm}|}
\hline
\textbf{Field} & \textbf{Track 1} & \textbf{Track 2} & \textbf{Track 3} & \textbf{Effect Estimation} & \textbf{Transportability} & \textbf{Mechanism Adjudication} \\
\hline
Causal Inf. & Causal discovery & Causal abstraction & Model testing & ATE / CATE & Transportability & Equiv. models \\
\hline
Neuroscience & Circuit mapping & Functional localization & Circuit dissection & Perturbation quant. & Cross-condition gen. & --- \\
\hline
Psychometrics & Exploratory FA & Construct oper. & Confirmatory FA & Effect size est. & Measurement invariance & Model comparison \\
\hline
Pharmacology & Target discovery & Target characterization & Proof of mechanism & Dose-response & Phase III generalization & --- \\
\hline
Philosophy of Science & Abduction & Natural kinds & H-D testing & Parameter estimation & Novel prediction & Crucial experiment \\
\hline
\end{tabular}
\end{table}

% Alternative cross-field view: V1-V4 as rows, fields as columns
\begin{table}[t]
\centering
\small
\renewcommand{\arraystretch}{1.08}
\caption{Cross-field mapping (alternative): scoring views mapped to disciplinary methods.}
\label{tab:crossfield_alt}
...
\paragraph{A worked example.} Consider the IOI claim that the three Name Mover heads (9.9, 9.6, 10.0) copy the indirect object token to the final position. The spec encodes one positive prediction---ablating the Name Mover role reduces logit difference by at least 0.2 relative to the full-model baseline---and one negative control---ablating Name Movers does not affect upstream S-Inhibition activation by more than 0.05. When \texttt{verify(spec)} runs, role ablation drops logit difference by 0.34 (positive prediction passes) and shifts S-Inhibition activation by 0.02 (negative control passes). The verifier returns a confirmation rate of 1.0 on this prediction pair, a claim ceiling at the implementational-functional mode, and a tier of Mechanistically Supported once aggregated with the remaining six predictions and five controls. A failure on either the positive or the negative side is informative: a missed positive prediction caps the claim ceiling at a lower mode, while a missed negative control flags a specificity failure (I3) that no faithfulness number can paper over.
...
\begin{figure}[htbp]
\centering
\IfFileExists{figures/fig_radar_wrapped_v12.png}{%
  \includegraphics[width=0.82\linewidth]{fig_radar_wrapped_v12.png}%
}{%
  \fbox{\parbox{\linewidth}{\centering Missing figure: fig_radar_wrapped_v12.png\\Run gen_radar_wrapped_v12.py to generate.}}%
}
\caption{Per-claim criterion coverage visualized as a Harvey-style 4-level diagram (v12). This figure summarizes which validity criteria each case study meets, providing a compact view of strengths and weaknesses across the 13 evaluated circuits.}
\label{fig:harvey_v12}
\end{figure}
\FloatBarrier
...
% --- Placeholder entries to satisfy unresolved citations ---
\bibitem[Karvonen(2025)]{karvonen2025saebench}
Karvonen, A., et~al.
\newblock SAEBench: Audit of sparse autoencoder metrics.
\newblock Preprint (report), 2025. (Placeholder entry — replace with full citation.)

\bibitem[Zheng(2023)]{zheng2023judging}
Zheng, E.
\newblock LLM-as-judge: Open-ended evaluation methods.
\newblock Preprint (report), 2023. (Placeholder entry — replace with full citation.)

\end{thebibliography}

% ============================================================
% --- Supplementary tables moved here from main text ---
\section{Supplementary Tables}
\label{app:supp-tables}

% Criteria: wide two-column table, with horizontal rules between rows
\begingroup
\hyphenpenalty=10000
...
\centering
\small
\caption{Operational criteria (moved from main text).}
\label{tab:criteria_app}
\renewcommand{\arraystretch}{1.08}
...
% Verdict tiers: table with lines between rows for readability (already moved)
\begin{table}[p]
\centering
\small
\caption{Verdict tiers and brief requirements (moved to appendix).}
\label{tab:verdicts_app}
...
% Scale table (moved to appendix) with row separators
\begin{table}[p]
\centering
\small
\caption{Current scale of the implementation (moved to appendix).}
\label{tab:scale_app}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{|l|r|}
\hline
