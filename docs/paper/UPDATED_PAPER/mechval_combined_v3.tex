...
Each validity type is operationalized through concrete criteria that together produce a structured validity profile (\Cref{tab:criteria}). For each criterion, a claim is assessed as confirmed, partially confirmed, or unmet based on the available published evidence. The criteria are designed so that each one identifies a specific way a mechanistic claim can fail to be well-supported; together, they characterize the full evidential basis rather than collapsing it into a single score.

\paragraph{Construct validity (C1--C5)} asks whether the claim is well-defined enough to be tested. C1 (falsifiability) requires that the claim specify what evidence would refute it---a requirement routinely violated when circuit labels are assigned post-hoc without pre-registered predictions. C2 (structural plausibility) asks whether the proposed mechanism is physically realizable in the architecture; for example, a claim that a single attention head implements multi-step reasoning fails C2 because the architecture constrains each head to a single bilinear interaction. C3 (task specificity) and C4 (minimality) address whether the mechanism is specific to the claimed behavior and whether it includes only necessary components. C5 (convergent validity) asks whether independent methods agree---a circuit identified through activation patching alone is weaker than one confirmed by both patching and weight-space analysis.

\paragraph{Internal validity (I1--I5)} establishes whether the evidence supports the causal claim. The most consequential distinction is between I1 (necessity) and I2 (sufficiency). Most published circuits demonstrate necessity---ablating the circuit degrades performance---but not sufficiency: running the circuit in isolation does not reproduce the behavior. This gap is the single most common barrier between tiers. I3 (specificity) asks whether the circuit is selective: does ablating the IOI circuit degrade \emph{only} indirect object identification, or does it also degrade unrelated tasks? I4 (consistency) requires that results hold across random seeds and data splits. I5 (confound control) asks whether alternative explanations have been ruled out---for instance, whether an apparent ``knowledge neuron'' effect might be explained by simpler token-frequency statistics.

\paragraph{Measurement validity (M1--M6)} asks whether the metrics themselves are trustworthy. M1 (reliability) requires that repeated measurements give the same answer; M3 (baseline separation) requires that scores exceed what random baselines achieve. These criteria expose a widespread problem: SAEBench found that two of six standard SAE evaluation metrics fail all three diagnostic tests (reseed stability, ground-truth correlation, discriminability). M4 (sensitivity) asks whether the metric can detect known-true effects, guarding against measures that are simply insensitive. M6 (construct coverage) asks whether the metric actually measures what it claims to---logit difference, for example, measures prediction shift at one token position but is often interpreted as a measure of circuit faithfulness.

\paragraph{External validity (E1--E6)} asks whether the mechanism generalizes beyond the original experimental conditions. E1 (intervention reach) is particularly revealing: \citet{miller2024transformer} showed that the IOI circuit's 87\% faithfulness figure is specific to mean ablation; under other ablation methods, faithfulness drops below 50\%. The mechanism is real, but the causal claim is method-conditional in a way the original paper did not characterize. E2 (graded response) requires dose-response evidence: partial ablation should produce partial effects, not all-or-nothing collapse. E6 (cross-architecture) asks whether the mechanism appears in models beyond the one studied---a criterion that sharply separates well-replicated mechanisms like induction heads from mechanisms studied in only one model.

\paragraph{Interpretive validity (V1--V5)} asks whether the natural-language interpretation is justified by the evidence. V4 (alternative exclusion) is the most commonly failed criterion: a head labeled a ``name mover'' based on direct logit attribution could equally be a ``high-salience token copier'' or a ``position-sensitive repeater.'' The label carries theoretical implications beyond what the evidence supports. V5 (scope honesty) asks whether the claim acknowledges its limitations---for instance, whether results from toy models are presented as general findings or appropriately qualified.

\begin{table}[t]
\centering
\tighttable
\caption{The 27 operational criteria organized by validity type. Each criterion specifies a concrete test that a mechanistic claim either passes, partially passes, or fails.}
\label{tab:criteria}
\small
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{fig_harvey_4level_v19.png}
\caption{Per-criterion validity profile for 13 published MI results, sorted by Claim Validity Score (CVS). Each pie chart represents one validity dimension; wedge colors indicate the status of individual criteria within that dimension (green: confirmed; light green: semi-confirmed; pink: mixed; red: disconfirmed; white: untested). Colored squares and scores to the left of each row indicate the overall verdict tier.}
\label{fig:harvey}
\end{figure}

\subsection{Verdict Assignments}

\Cref{tab:case-studies} presents the verdict assignments and \Cref{fig:harvey} shows the per-criterion validity profile for each circuit. The key finding is that the tier system produces meaningful discrimination: the 13 claims span the full range from Proposed to Triangulated, with verdict differences traceable to specific missing evidence rather than arbitrary scoring.
...
\midrule
\multirow{2}{*}{Triangulated} & Induction Heads & Broad replication across scales; thick evidential network \\
& Grokking & All criteria met within toy scope; mathematically complete algorithm specification \\
\midrule
\multirow{4}{*}{\parbox{2.3cm}{Mechanistically\\Supported}} & Greater-Than & Strong structural plausibility (C2); limited prompt generalization \\
& Copy Suppression & Unusually clean specificity; sufficiency partially established \\
& IOI Circuit & Method-conditional faithfulness (87\% mean ablation, $<$50\% other methods); specificity untested \\
& Superposition & Toy-model-only evidence; criteria semi-confirmed rather than confirmed \\
\midrule
...
\paragraph{The toy-model distinction.} Both Grokking and Superposition are studied exclusively in toy models, but the framework distinguishes them. Grokking reaches Triangulated because the evidence is mathematically complete: every weight matrix entry is predicted by a closed-form Fourier formula, leaving no ambiguity about the algorithm. Superposition reaches Mechanistically Supported because its criteria are semi-confirmed rather than confirmed---the theory's predictions hold in toy settings but remain inferred rather than demonstrated in real pretrained models. Neither reaches Validated, which requires evidence from production-scale models. The distinction illustrates that completeness within a restricted scope is a genuine achievement while honestly representing the gap to real-model confirmation.
...
The Superposition paper \citep{elhage2022superposition} was excluded from the automated pipeline evaluation because its purely analytical methodology maps poorly onto criteria designed for empirical circuit-discovery papers. The pipeline scores it at Proposed, missing the substantial theoretical evidence that the manual case-study analysis captures. The manual assessment (\Cref{sec:case-studies}) assigns Superposition to Mechanistically Supported based on semi-confirmed criteria across toy-model evidence. This discrepancy illustrates a limitation of the automated pipeline: theoretical papers require evaluation norms that the current extraction prompts do not encode.
