...
\usepackage{titlesec}
\titlespacing*{\subsection}{0pt}{2.0ex plus .2ex}{0.8ex}
...
The 27 criteria also formalize common failure modes observed in published MI research (\Cref{tab:failures}).
...
\centering
\small
\caption{Seven description modes, ordered from most committal (computational) to least committal (statistical). Higher modes require all evidence from lower modes plus bridging evidence.}
\label{tab:modes}
...
Many metrics correspond to entire experimental methodologies in their source fields; the framework represents them as metrics to provide a common unit of comparison across evidence families and allow them to be scored against the same criteria. The full inventory for all six families is in \Cref{appendix:metrics}.

% TODO: In appendix, add the same per-metric breakdown (with descriptions from the site)
% for structural, information-theoretic, behavioral, and representational families.
...
\centering
\small
\caption{Twenty-seven criteria across five validity types. Each criterion has a concrete test; a claim either passes, partially passes, or fails, producing a structured validity profile.}
\label{tab:criteria}
\renewcommand{\arraystretch}{1.15}
...
We call these traditions \emph{lenses} (\Cref{fig:foundations}). A lens is not a method or a metric---it is an analytical vocabulary that shapes which questions you ask and how you interpret the answers. Five lenses map to the five validity types; a sixth---mechanistic interpretability itself---grounds interpretive validity, because no other field faces the specific problem of assigning human-interpretable labels to neural network components.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{foundations-hierarchy.pdf}
\caption{Six theoretical foundations, each grounding a specific validity type. Causal inference and neuroscience both contribute to internal validity through complementary approaches (formal frameworks and experimental designs, respectively).}
\label{fig:foundations}
\end{figure}

Each lens answers a core question about the claim: philosophy of science asks \emph{is the entity you named a real thing?} Causal inference and neuroscience ask \emph{does the component implement the computation?} Psychometrics asks \emph{can you trust the number?} Pharmacology asks \emph{does it generalize?} And mechanistic interpretability asks \emph{does the interpretation match the evidence?}

The lens pages are reference material---they explain \emph{why} each criterion exists, not how to apply it. A researcher evaluating a claim follows the pipeline; a researcher who wants to understand why a criterion matters, or who wants to design a new experiment to address a gap, reads the relevant lens.
...
\bibitem[Black and Leff(1983)]{black1983operational}
Black, J.~W. and Leff, P.
\newblock Operational models of pharmacological agonism.
\newblock \emph{Proceedings of the Royal Society B}, 220(1219):141--162, 1983.

\bibitem[Campbell and Fiske(1959)]{campbell1959convergent}
...
\bibitem[Craver(2007)]{craver2007explaining}
Craver, C.~F.
\newblock \emph{Explaining the Brain: Mechanisms and the Mosaic Unity of Neuroscience}.
...
\bibitem[Duhem(1906)]{duhem1906theorie}
Duhem, P.
\newblock \emph{La Th{\'e}orie Physique: Son Objet, Sa Structure}.
\newblock Marcel Rivi{\`e}re, 1906.
...
\bibitem[Hill(1910)]{hill1910possible}
Hill, A.~V.
\newblock The possible effects of the aggregation of the molecules of haemoglobin on its dissociation curves.
\newblock \emph{Journal of Physiology}, 40:iv--vii, 1910.

\bibitem[Kenakin(2004)]{kenakin2004functional}
Kenakin, T.
\newblock Principles: Receptor theory in pharmacology.
\newblock \emph{Trends in Pharmacological Sciences}, 25(4):186--192, 2004.
...
\bibitem[Rang(2006)]{rang2006drug}
Rang, H.~P.
\newblock The receptor concept: Pharmacology's big idea.
\newblock \emph{British Journal of Pharmacology}, 147(S1):S9--S16, 2006.
...
\bibitem[Shallice(1988)]{shallice1988neuropsychology}
Shallice, T.
\newblock \emph{From Neuropsychology to Mental Structure}.
\newblock Cambridge University Press, 1988.

\bibitem[Stephenson(1956)]{stephenson1956modification}
Stephenson, R.~P.
\newblock A modification of receptor theory.
\newblock \emph{British Journal of Pharmacology}, 11(4):379--393, 1956.
...
\bibitem[M\'eloux et~al.(2025)]{meloux2025everything}
M\'eloux, M., Maniu, S., Portet, F., and Peyrard, M.
\newblock Everything, Everywhere, All at Once: Is Mechanistic Interpretability Identifiable?
...
\citep{miller2024transformer} find the faithfulness numbers to be metho-conditional, and \citep{meloux2025everything} find alternative head sets achieving comparable faithfulness.
...
%\centering
%\small
%\caption{Measurement metrics (F): calibration checks for the trustworthiness of other metrics.}
%\renewcommand{\arraystretch}{1.12}
%\begin{tabular}{@{}lp{10cm}@{}}
%\toprule
%\textbf{Metric} & \textbf{Description} \\
...
%\begin{center}
%\captionof{table}{Causal metrics: the effect of interventions on the model's computation.}
%\begin{tabular}{@{}lp{10cm}@{}}
%\toprule
%\textbf{Metric} & \textbf{Description} \\
%\midrule
