...
def draw_single_radar(ax, vals, color='#64748b', show_dim_labels=True, title=None, title_pad=12, label_fontsize=8.0, title_fontsize=9.5):
...
def make_vertical_grouped_radars(case_studies, filename, show_dim_labels=True, title_pad_base=14):
    # group by tier
    ...
    for tier_idx, (tier_name, items) in enumerate(tier_groups):
        ...
        for j, (name, verdict, c, i, m, e, v) in enumerate(items):
            ...
            title_pad = title_pad_base + tier_idx * 5
            ...
            draw_single_radar(ax, [c, i, m, e, v], color=color, show_dim_labels=show_dim_labels,
                              title=title_text, title_pad=title_pad)
    ...
    fig.subplots_adjust(hspace=0.9, wspace=0.6, left=0.03, right=0.98, top=0.98, bottom=0.12)
...
def make_no_labels_with_example_legend(case_studies, filename):
    ...
    for tier_idx, (tier_name, items) in enumerate(tier_groups):
        ...
        for j, (name, verdict, c, i, m, e, v) in enumerate(items):
            ...
            draw_single_radar(ax, [c, i, m, e, v], color=color, show_dim_labels=False, title=name, title_pad=12)
        for empty_col in range(ncols, max_cols):
            ax = fig.add_subplot(n_rows, max_cols, tier_idx * max_cols + empty_col + 1, polar=True)
            ax.axis('off')
    ...
    draw_single_radar(ax_inset, example_vals, color='#64748b', show_dim_labels=True,
                      ...
                      label_fontsize=9.0, title_fontsize=9.0)
    ...
    fig.subplots_adjust(hspace=0.9, wspace=0.6, left=0.03, right=0.98, top=0.96, bottom=0.12)
    outpath = OUTDIR / filename
    fig.savefig(str(outpath), bbox_inches='tight', dpi=300, facecolor='white')
    plt.close(fig)
    print('Saved', outpath)
...
if __name__ == '__main__':
    ...
    make_no_labels_with_example_legend(cs, 'fig_radar_no_labels_v6.png')
    make_vertical_grouped_radars(cs, 'fig_radar_vertical_v6.png', show_dim_labels=True, title_pad_base=12)
