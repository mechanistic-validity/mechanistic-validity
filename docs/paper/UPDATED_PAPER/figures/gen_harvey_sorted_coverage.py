...
# Count NOs per circuit and sort (fewest NOs first)
no_counts = {}
for name, cdata in criteria_raw.items():
    no_counts[name] = sum(1 for v in cdata.values() if v == 0)

circuit_order = sorted(criteria_raw.keys(), key=lambda n: (no_counts[n], -sum(criteria_raw[n].values())))

# Print the ranking
for name in circuit_order:
    yes_c = sum(1 for v in criteria_raw[name].values() if v == 1)
    par_c = sum(1 for v in criteria_raw[name].values() if v == 0.5)
    no_c = no_counts[name]
    print(f"{name:25s}  YES={yes_c:2d}  PARTIAL={par_c:2d}  NO={no_c:2d}  tier={tier_map[name]}")
...
ax.set_xlim(-7.0, (n_dims - 1) * col_spacing + radius + 0.5)
...
plt.savefig('fig_harvey_sorted_by_coverage.png', dpi=200, bbox_inches='tight', facecolor='white')
print('saved fig_harvey_sorted_by_coverage.png')
