...
tier_rank = {"Triangulated": 0, "Mech. Supported": 1, "Caus. Suggestive": 2, "Proposed": 3}
...
# Compute counts
stats = {}
for name, cdata in criteria_raw.items():
    yes_c = sum(1 for v in cdata.values() if v == 1)
    no_c = sum(1 for v in cdata.values() if v == 0)
    stats[name] = (yes_c, no_c)

# 3 sort orders
tier_order = ["Induction Heads","Grokking","Superposition","IOI Circuit",
              "Greater-Than","Copy Suppression","Successor Heads",
              "Docstring Circuit","SAE Features","Othello World Model",
              "Knowledge Neurons","Probing Classifiers","Gender Bias Circuits"]

coverage_order = sorted(criteria_raw.keys(), key=lambda n: (stats[n][1], -stats[n][0]))
yes_order = sorted(criteria_raw.keys(), key=lambda n: (-stats[n][0], stats[n][1]))

sorts = [
    ("fig_harvey_sort_tier.png", tier_order, "Sorted by Tier"),
    ("fig_harvey_sort_coverage.png", coverage_order, "Sorted by Fewest Gaps"),
    ("fig_harvey_sort_confirmed.png", yes_order, "Sorted by Most Confirmed"),
]

for fname, order, title in sorts:
    n_rows = len(order)
    col_spacing = 1.3
    row_spacing = 1.1
    radius = 0.38
    ...
    # Title
    ax.text((4 * col_spacing) / 2, n_rows * row_spacing + 1.3, title,
            fontsize=12, fontweight='bold', ha='center', va='bottom', color='#1e293b')
    ...
    for i, name in enumerate(order):
        ...
        for dim in range(5):
            ...
            for seg, ckey in enumerate(keys):
                ...
                val = cdata[ckey]
                if val == 1:
                    fc, alpha = fill_color, 0.85
                elif val == 0.5:
                    fc, alpha = fill_color, 0.3
                else:
                    fc, alpha = 'white', 1.0
    ...
    ly = -1.1
    ax.add_patch(mpatches.Wedge((-1.5, ly), 0.2, 0, 360,
                 facecolor='#94a3b8', alpha=0.85, edgecolor='white', linewidth=1))
    ax.text(-1.1, ly, 'YES', fontsize=9, va='center', color='#475569', fontweight='bold')
    ax.add_patch(mpatches.Wedge((0.2, ly), 0.2, 0, 360,
                 facecolor='#94a3b8', alpha=0.3, edgecolor='#cbd5e1', linewidth=1))
    ax.text(0.6, ly, 'PARTIAL', fontsize=9, va='center', color='#475569', fontweight='bold')
    ax.add_patch(plt.Circle((2.2, ly), 0.2,
                 facecolor='white', edgecolor='#cbd5e1', linewidth=1))
    ax.text(2.6, ly, 'NO', fontsize=9, va='center', color='#475569', fontweight='bold')
