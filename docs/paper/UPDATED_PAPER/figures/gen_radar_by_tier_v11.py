...
def draw_single_radar(ax, vals, color='#64748b', show_dim_labels=True, title=None, title_pad=12,
                      label_fontsize=8.0, label_fontweight='normal', title_fontsize=9.0):
    ...
    if title:
        ax.set_title(title, fontsize=title_fontsize, fontweight='700', color='#0f172a',
                     pad=title_pad, linespacing=1.6)


def make_vertical_grouped_radars(case_studies, filename, show_dim_labels=True, title_pad_base=12):
    # group by tier
    ...
    # decrease vertical space per row (more compact than v8/v7)
    fig_h = 2.0 * n_rows
    ...
    for tier_idx, (tier_name, items) in enumerate(tier_groups):
        ...
        for j, (name, verdict, c, i, m, e, v) in enumerate(items):
            ...
            title_pad = title_pad_base + 3 + tier_idx * 2
            # include cvs score under the name with slight extra spacing
            ...
            # For the top tier row, nudge the axes downward slightly to visually center plots
            if tier_idx == 0:
                pos = ax.get_position()
                # lower by a small fraction of figure height (smaller delta for compact layout)
                delta = 0.01
                newpos = [pos.x0, max(0.0, pos.y0 - delta), pos.width, pos.height]
    ...
    # legend with CVS ranges
    tier_ranges = {
        "Triangulated": "Triangulated (8.0–10.0)",
        "Mech. Supported": "Mech. Supported (6.0–7.9)",
        "Caus. Suggestive": "Caus. Suggestive (4.0–5.9)",
        "Proposed": "Proposed (<4.0)",
    }
    handles = [mpatches.Patch(
        facecolor=(harvey.tier_colors.get(t) if harvey and hasattr(harvey, 'tier_colors') else TIER_COLORS[t]),
        edgecolor='none', label=tier_ranges.get(t, t))
        for t in TIER_ORDER if groups.get(t)]
    fig.legend(handles=handles, loc='lower center', ncol=2, fontsize=9,
               frameon=False, bbox_to_anchor=(0.5, -0.04))

    # adjust margins — more hspace so titles don't crowd the graph below
    fig.subplots_adjust(hspace=1.0, wspace=0.5, left=0.03, right=0.98, top=0.94, bottom=0.08)
...
def make_no_labels_with_example_legend(case_studies, filename):
    ...
    for tier_idx, (tier_name, items) in enumerate(tier_groups):
        ...
        for j, (name, verdict, c, i, m, e, v) in enumerate(items):
            ...
            draw_single_radar(ax, [c, i, m, e, v], color=color, show_dim_labels=False, title=name, title_pad=10,
    ...
    fig.subplots_adjust(hspace=0.6, wspace=0.5, left=0.03, right=0.98, top=0.96, bottom=0.10)
    outpath = OUTDIR / filename
    fig.savefig(str(outpath), bbox_inches='tight', dpi=300, facecolor='white')
    plt.close(fig)
    print('Saved', outpath)
...
if __name__ == '__main__':
    all_cs = build_case_studies_from_harvey()

    # Selective subset: 3 green, 3 yellow, 2 red
    keep = {
        "Grokking", "Induction Heads",
        "IOI Circuit", "Greater-Than", "Superposition",
        "Successor Heads", "Knowledge Neurons", "SAE Features",
        "Probing Classifiers", "Gender Bias Circuits",
    }
    if all_cs:
        cs = [c for c in all_cs if c[0] in keep]
    else:
        cs = [
            ("IOI Circuit", "Mech. Supported", 2, 2, 2, 2, 2),
            ("Greater-Than", "Mech. Supported", 3, 2, 2, 1, 2),
            ("Superposition", "Mech. Supported", 2, 2, 2, 1, 2),
            ("Successor Heads", "Caus. Suggestive", 2, 2, 1, 2, 1),
            ("Knowledge Neurons", "Caus. Suggestive", 2, 2, 1, 2, 1),
            ("SAE Features", "Caus. Suggestive", 1, 1, 1, 1, 0),
            ("Probing Classifiers", "Proposed", 1, 0, 1, 0, 1),
            ("Gender Bias Circuits", "Proposed", 1, 0, 1, 0, 0),
        ]

    make_vertical_grouped_radars(cs, 'fig_radar_selective_v3.png', show_dim_labels=True, title_pad_base=12)
