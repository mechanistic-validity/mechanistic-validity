"""Harvey 4-level v17: v11 right-middle base + grid lines + alternating row stripes."""
import matplotlib
matplotlib.use('Agg')
...
ax.set_xlim(-10.0, circles_right + 4.0)
...
grid_left = -0.7 * col_spacing
grid_right = 4 * col_spacing + col_spacing / 2
for i in range(n_rows):
    if i % 2 == 1:
        y = (n_rows - 1 - i) * row_spacing
        ax.add_patch(mpatches.Rectangle(
            (grid_left, y - row_spacing / 2), grid_right - grid_left, row_spacing,
...
for i in range(n_rows + 1):
    if i < n_rows:
        ...
        gy = -row_spacing / 2
    ax.axhline(y=gy, xmin=0, xmax=1, color=grid_color, linewidth=grid_lw,
               zorder=0)

col_positions = [j * col_spacing for j in range(5)]
for j in range(6):
    if j == 0:
        gx = col_positions[0] - col_spacing / 2
    elif j == 5:
        gx = col_positions[4] + col_spacing / 2
    else:
        gx = (col_positions[j - 1] + col_positions[j]) / 2
    ax.axvline(x=gx, color=grid_color, linewidth=grid_lw, zorder=0)
...
out = OUTDIR / "fig_harvey_4level_v17.png"
