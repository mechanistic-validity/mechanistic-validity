...
def make_vertical_grouped_radars(case_studies, filename, show_dim_labels=True, title_pad_base=18):
    ...
    for tier_idx, (tier_name, items) in enumerate(tier_groups):
        ...
        for j, (name, verdict, c, i, m, e, v) in enumerate(items):
            ...
            if tier_idx == 0:
                pos = ax.get_position()
                # lower by a small fraction of figure height
                delta = 0.03
    ...
    fig.subplots_adjust(hspace=1.05, wspace=0.6, left=0.03, right=0.98, top=0.995, bottom=0.10)
...
if __name__ == '__main__':
    ...
    make_no_labels_with_example_legend(cs, 'fig_radar_no_labels_v8.png')
    make_vertical_grouped_radars(cs, 'fig_radar_vertical_v8.png', show_dim_labels=True, title_pad_base=18)
