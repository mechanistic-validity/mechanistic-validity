...
def make_wrapped(case_studies, cols=4, filename='fig_radar_wrapped_v12.png'):
    ...
    for idx, (name, verdict, c, i, m, e, v) in enumerate(case_studies):
        ...
        # Compute title and verdict subtitle; place title higher and add colored verdict line below
        # Compute title with colon format and draw radar, then add verdict label
        if harvey and hasattr(harvey, 'cvs_scores'):
            score = harvey.cvs_scores.get(name, None)
            title_text = f"{name}: {score:.1f}/10" if score is not None else name
        else:
            title_text = name
        # draw radar without title then place custom title and verdict lines for finer control
        draw_radar(ax, [c,i,m,e,v], color, show_labels=False, title=None)
        # place main title higher above the axes
        ax.set_title(title_text, fontsize=10, fontweight='700', pad=18)
        # Map tier display names to full forms
        DISPLAY_TIERS = {
        ...
        verdict_display = DISPLAY_TIERS.get(verdict, verdict)
        # place verdict label slightly below the title, colored to match the radar
        ax.text(0.5, 1.02, verdict_display, transform=ax.transAxes, ha='center', va='bottom', fontsize=8, fontweight='700', color=color)
    # compact boxed legend removed; place legend centered near bottom but raised
    # Use full display names for legend labels
    DISPLAY_TIERS = {
        'Mech. Supported': 'Mechanistically Supported',
        'Caus. Suggestive': 'Causally Suggestive',
        'Proposed': 'Proposed',
    ...
    handles = []
    for t in TIER_ORDER:
        if any(cs[1]==t for cs in case_studies):
            color_t = (harvey.tier_colors.get(t) if harvey and hasattr(harvey, 'tier_colors') else TIER_COLORS[t])
            label = DISPLAY_TIERS.get(t, t)
            handles.append(mpatches.Patch(facecolor=color_t, edgecolor='none', label=label))
    labels = [h.get_label() for h in handles]
    # reduce bottom padding so legend sits closer to last row
    fig.subplots_adjust(hspace=0.6, wspace=0.5, left=0.03, right=0.98, top=0.96, bottom=0.12)
    # place legend centered, slightly above bottom (y in figure coords)
    fig.legend(handles=handles, labels=labels, loc='lower center', bbox_to_anchor=(0.5, 0.12), ncol=len(handles), frameon=False, fontsize=9)
...
if __name__ == '__main__':
    ...
    make_wrapped(cs, cols=4, filename='fig_radar_wrapped_v12.png')
