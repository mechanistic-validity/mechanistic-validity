...
# CVS scores (debiased) from the paper data
cvs_scores = {
...
status_colors = {
    1.0:  "#16a34a", 0.7:  "#86efac", 0.3:  "#fdba74",
    -1.0: "#ef4444", 0.0:  "white",
}
...
ax.set_xlim(-7.5, (5 - 1) * col_spacing + radius + 0.5)
ax.set_ylim(-2.8, n_rows * row_spacing + 1.2)
ax.set_aspect('equal')
ax.axis('off')

# Column headers — black, closer to circles
...
for i, name in enumerate(circuit_order):
    ...
    # CVS score in tier color instead of square
    score_x = -0.75
    ax.text(score_x, y, f"{cvs:.1f}", fontsize=11, ha='center', va='center',
            fontweight='bold', color=tier_colors[tier])

    # Circuit name — bigger font
    ax.text(score_x - 0.4, y, name, fontsize=12, ha='right', va='center',
...
tiers = [("Proposed","#ef4444"), ("Caus. Suggestive","#f59e0b"),
         ("Mech. Supported","#10b981"), ("Triangulated","#3b82f6")]
tier_labels = ["Proposed", "Caus. Suggestive", "Mech. Supported", "Triangulated"]
for idx, (tname, tcol) in enumerate(tiers):
    ...
    ax.text(tx + 0.25, ty, tier_labels[idx], fontsize=9, va='center', color='#475569')
