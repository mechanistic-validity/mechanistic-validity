...
% ============================================================
\section{Overview: From Claim to Verdict}
\label{sec:overview}

Consider a concrete claim: ``The IOI circuit uses three name-mover heads (9.9, 9.6, 10.0) to copy the indirect object to the output position.'' This claim appears in a widely-cited paper, is supported by ablation experiments, and has a clear mechanistic narrative. What would it take to know whether this claim is actually valid?

The claim implicitly encodes a causal graph. Name-mover heads receive input from S-inhibition heads, which receive input from induction heads, which receive input from duplicate-token detectors. The claim says these edges exist. But it also implicitly says that certain edges do \emph{not} exist---for example, that ablating name-mover heads does not disrupt upstream S-inhibition heads (the information flows one way, not both). These implicit non-edges are where most mechanistic claims break down, because they are rarely tested.

% [FIGURE PLACEHOLDER: ClaimSpec graph for IOI circuit. Left: the claimed DAG
% with nodes (DTH, PTH, IND, S-Inh, NM, NegNM) and directed edges. Right:
% the same DAG with dashed non-edges representing negative controls --- e.g.,
% NM -/-> S-Inh. One non-edge highlighted in red to show a hypothetical failure.]

The \mechval{} framework makes this implicit graph explicit as a \emph{ClaimSpec graph}: a directed acyclic graph whose nodes are computational roles, whose edges are claimed dependencies with testable positive predictions, and whose non-edges are claimed independences with testable negative controls. Pre-registering the graph before running experiments is what separates a falsifiable claim from a post-hoc narrative---a narrative that was retrofitted to match observations can always redraw its edges to fit, but a pre-registered graph with negative controls on its non-edges can actually fail.
