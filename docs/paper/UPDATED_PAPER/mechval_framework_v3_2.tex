...
The \mechval{} framework makes this implicit graph explicit as a \emph{ClaimSpec graph} (\Cref{fig:claimspec_ioi}): a graph whose nodes are claimed computational roles, whose edges are claimed dependencies with testable positive predictions, and whose non-edges are claimed independences with testable negative controls. The graph represents the structure of the \emph{claim}, not the structure of the model.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{fig_claimspec_ioi.png}
\caption{ClaimSpec graph for the IOI circuit. Solid arrows are positive predictions (``ablating this role degrades downstream behavior''). Dashed red arrows with \texttimes{} are negative controls (``ablating this role does \emph{not} affect upstream components''). The graph encodes what the claim asserts and what it rules out.}
\label{fig:claimspec_ioi}
\end{figure}

The ClaimSpec graph is not specific to multi-role circuits. \Cref{fig:claimspec_knowledge} shows the graph for knowledge neurons: a single node with one positive prediction (ablating the neuron degrades recall of a specific fact) and four untested non-edges (does it also degrade unrelated facts? general fluency? other MLP neurons?). Without testing the non-edges, ``knowledge neuron'' is indistinguishable from ``middle-layer neuron that disrupts everything when ablated.''

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{fig_claimspec_knowledge.png}
...
Most revealing is the case where the ClaimSpec graph exposes a problem with the claim itself, not just missing evidence. \Cref{fig:claimspec_gender} shows gender bias circuits: the claim requires that ``gender bias'' and ``gender knowledge'' be separable constructs implemented by different components. But both map to the same attention heads. The required non-edge between them does not exist---you cannot ablate bias without ablating knowledge. The claim graph collapses, revealing that the construct itself is incoherent (a C4 discriminant validity failure). No amount of additional evidence can fix this; the question needs reformulation.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{fig_claimspec_gender.png}
\caption{ClaimSpec graph for gender bias circuits. The claim requires separability between bias and knowledge (red dashed non-edge), but both map to the same heads. The graph collapses---a construct-level failure that no experiment can resolve.}
\label{fig:claimspec_gender}
\end{figure}

Pre-registering the graph before running experiments is what separates a falsifiable claim from a post-hoc narrative---a narrative that was retrofitted to match observations can always redraw its edges to fit, but a pre-registered graph with negative controls on its non-edges can actually fail.
...
Every tier carries a specific epistemic meaning: a claim at \emph{Proposed} is not a failed claim but one whose evidential status---and the path to strengthening it---is precisely characterized. The verdict tells researchers exactly what additional evidence would advance the claim: a Proposed claim needs causal intervention (I1) to reach Causally Suggestive; a Causally Suggestive claim needs sufficiency evidence (I2) and method consistency (E1) to reach Mechanistically Supported. \Cref{fig:radar} illustrates this progression across ten published circuits.
