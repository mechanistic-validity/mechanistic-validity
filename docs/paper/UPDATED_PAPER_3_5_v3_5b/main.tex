...
% ============================================================
\begin{abstract}
A mechanistic claim---\emph{this component implements this function}---can be supported by real data, produced by a reliable method, and still be evidentially unwarranted. Most published mechanistic claims rest on a single type of evidence: an intervention was performed and behavior changed. This is a causal observation---and it is real---but it is not enough to conclude that the component \emph{implements} the computation, that the measurement is \emph{trustworthy}, that the finding \emph{generalizes}, or that the explanation is stated at the \emph{right level of abstraction}. Each of these is a distinct way a claim can fail, and each requires its own evidence. Fields that make mechanistic claims rigorously evaluate their \emph{methods}. They rarely evaluate whether the \emph{claims built on those methods} are evidentially warranted. These are different questions, and confusing them is systematic, not accidental.

We introduce Mechanistic Validity (\mechval{}), a formal framework that makes these distinctions explicit. \mechval{} draws on philosophy of science, measurement theory, causal inference, and validation methodology developed in neuroscience, pharmacology, and molecular genetics---fields that learned, usually by getting burned, what happens when each layer of inference is skipped. The framework evaluates claims through a seven-layer pipeline---from \emph{description mode} (what kind of explanation is being offered, declared before analysis, not labeled after it) through \emph{evidence families}, \emph{metrics}, and \emph{criteria}, to five \emph{validity types} in dependency order, \emph{synthesis protocols}, and a \emph{verdict}. The five validity types encode a specific logical dependency: construct validity (is the claimed mechanism well-defined and distinguishable from its neighbors?) gates measurement validity (does the instrument reliably detect it?), which gates internal validity (is the relationship causal?), which gates external validity (does it generalize?), which gates interpretive validity (does the description match what was actually found?). An ambiguous construct cannot be measured. An unreliable metric cannot support a causal inference. A finding not established locally cannot be said to generalize. A narrative cannot be evaluated until the mechanism itself has been established. Evidence that skips a layer is not merely incomplete---it is uninterpretable.

\mechval{} operationalizes this structure as 30 criteria across the five validity types, producing a structured validity profile and a tier assignment---from Proposed through Causally Suggestive, Mechanistically Supported, and Triangulated, to Validated---for any mechanistic claim. The tier is determined by the weakest gating validity type, not the average. A continuous score would imply a precision the evidence does not support; the tier system names qualitative transitions. Each criterion is grounded in a specific intellectual tradition that developed it for a reason: falsifiability because philosophy of science spent a century learning what happens when claims cannot be disconfirmed; necessity-plus-sufficiency because neuroscience spent decades discovering that necessity alone does not establish implementation; dose-response because pharmacology learned that single-dose measurements hide compensatory dynamics.

We demonstrate the framework on 13 published mechanistic claims in neural network interpretability. The tier system discriminates sharply. Probing classifiers reach Proposed: they establish decodability without causal intervention. Induction heads reach Triangulated: structural, causal, and cross-context evidence converge. Gender bias circuits reach Disconfirmed: the construct itself fails discriminant validity, because the heads mediating ``bias'' are the same heads encoding grammatical gender, and no ablation regime reduces one without equally degrading the other. No published claim currently reaches Validated. The most-cited circuit in the field reports 87\% faithfulness under one ablation method and below 50\% under others; unconstrained nonlinear alignment achieves 100\% interchange-intervention accuracy on randomly initialized models; no published circuit has been replicated across model families under pre-registered protocols. The gap between what the field asserts and what the evidence warrants is structural, not idiosyncratic.
\end{abstract}
...
Double dissociation (I4) is almost never tested. A proper double dissociation would require showing that ablating circuit A breaks task X but not task Y, \emph{and} ablating circuit B breaks task Y but not task X. \citet{meloux2025everything} raise a more fundamental challenge: activation patching returns alternative head sets with comparable faithfulness scores for the same task. The ``IOI circuit'' is not the unique set of heads implementing indirect object identification---it is \emph{one of several} head sets that pass the same faithfulness threshold. This is Quine's underdetermination problem \citep{quine1951two} made concrete in mechanistic interpretability: the data (faithfulness scores) are consistent with multiple incompatible theories (circuits), and the standard experimental procedure cannot distinguish between them.

This result does not invalidate circuit discovery---it constrains what circuit discovery can claim. A circuit that passes faithfulness tests has demonstrated that it is \emph{a sufficient subnetwork}, not \emph{the mechanism}. The distinction matters: if three different head sets each recover 85\% of logit difference, the interesting question is not which one is ``the circuit'' but what they share (the intersection may be the genuine causal core) and how they differ (the symmetric differences may reflect the method's degrees of freedom). \mechval{} handles this through the Underdetermined verdict: when evidence is consistent with multiple competing mechanisms, the framework requires additional experiments that discriminate between rivals rather than selecting one post hoc. Reporting non-uniqueness explicitly---as \citet{meloux2025everything} do---is a stronger scientific contribution than suppressing it.
...
A circuit is not a bag of independently necessary components---it is a structured mechanism whose parts may cooperate or compensate. I6 asks whether circuit components interact non-additively.

\paragraph{I6a: Pairwise epistasis scores.} For all component pairs $(A, B)$ in circuit $C$, compute the epistasis score $\epsilon_{AB} = f(\text{ablate } A \cup B) - f(\text{ablate } A) - f(\text{ablate } B) + f(\emptyset)$, where $f$ is the task metric on the intact model. Positive $\epsilon$ indicates synergy (the pair cooperates beyond their individual contributions); negative $\epsilon$ indicates buffering (partial redundancy). An epistasis matrix with $|\epsilon_{AB}| < 0.05$ for all pairs indicates additive structure---the circuit is a list, not a mechanism. Significant non-zero entries ($|\epsilon_{AB}| > 0.1$) identify the circuit's functional joints: the edges where components are genuinely coupled.

\paragraph{I6b: Higher-order interaction test.} For a circuit with $K$ components, compute the full interaction contrast $\Delta^K = \sum_{S \subseteq C} (-1)^{|C|-|S|} f(\text{ablate } S)$. This is the $K$-way analog of the pairwise epistasis score, detecting irreducibly collective effects. For practical circuits ($K > 6$), approximate by sampling random subsets and testing whether the observed distribution of subset ablation effects deviates from the additive prediction (permutation test, $p < 0.01$).
...
\paragraph{What the field should do differently.}
