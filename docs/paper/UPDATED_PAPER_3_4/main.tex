...
We describe lessons from validity failures across sciences (\Cref{sec:motivation}), the framework (\Cref{sec:framework}), its theoretical foundations (\Cref{sec:foundations}), its application to 13 published circuits (\Cref{sec:case-studies}), and its implications for the field (\Cref{sec:discussion}).
...
\subsection{Operationalizing Under-Specified Criteria}
\label{sec:protocols}

Three criteria---V4 (interpretive inflation), I5 (confound control), and E1 (intervention reach)---are among the most commonly cited as important but the least well-operationalized in practice. We provide concrete, computable protocols for each, transforming them from checklist items into measurement procedures.
...
These nine protocols (three per criterion) are implemented as runnable evaluation scripts in the open-source \mechval{} codebase, following the same interface as the 84 metrics in \Cref{tab:metric-inventory}.

\subsection{Multi-Method Synthesis Protocols}
\label{sec:synthesis}

The criteria in \Cref{tab:criteria} include convergent validity (C3), which asks whether independent methods agree, and external validity criteria (E2--E4), which ask whether findings generalize. But the framework as presented does not specify \emph{how} to check multi-method agreement. This section provides three concrete synthesis procedures of increasing sophistication, addressing the operationalization gap identified in the tier promotion requirements (\Cref{tab:tier-promotion}).
...
C3 & Convergent validity & PC & Primarily ablation-based; limited cross-family evidence. RRA (\S\ref{sec:synthesis}) would quantify whether agreement exceeds chance \\
...
I5 & Confound control & U & Residual stream confounds not addressed; I5a (\S\ref{sec:protocols}) specifies confound battery for IOI (positional, frequency, syntactic, length) \\
...
E1 & Intervention reach & I & 87\% mean ablation; $<$50\% resample ablation \citep{miller2024transformer}; E1a (\S\ref{sec:protocols}) would test steering reach in the positive direction \\
...
V4 & Anthropomorphism check & PC & ``Name mover'' is descriptive; ``S-inhibition'' implies intent; V4b (\S\ref{sec:protocols}) specifies contrastive test to validate labels \\
...
The protocols in \S\ref{sec:protocols} provide concrete next steps for the highest-value untested criteria: (1) \textbf{I5 confound control}---run the I5a confound battery (positional, frequency, syntactic, length confounds) to determine whether the circuit is doing ``indirect object identification'' or ``copy the second proper noun''; (2) \textbf{V4 label validation}---run V4b contrastive label tests on ``name mover'' (equalize IO and S salience) and ``S-inhibition'' (test whether suppression is position- or identity-based); (3) \textbf{E1 intervention reach}---run E1a steering reach to test whether amplifying the circuit's activation direction \emph{increases} IOI performance proportionally, not just whether ablation decreases it; (4) \textbf{C3 convergent validity}---apply Dawid-Skene consensus (\S\ref{sec:synthesis}) across activation patching, weight analysis, and information-theoretic methods to determine which heads survive reliability-weighted voting. Additionally: (5) cross-method convergence---replicate the circuit via weight-space analysis; (6) cross-model replication---test whether the same circuit appears in GPT-2 Medium or Pythia.
