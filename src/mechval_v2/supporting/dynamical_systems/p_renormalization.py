"""Protocol PH_RG — Renormalization Group
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Dynamical Systems
Validity Type: Construct
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/cross_discipline/renormalization/
Family:       Cross-discipline (Physics)
Validity:     External — renormalization group paradigms applied to circuit structure

References:
    Wilson (1971) "Renormalization Group and Critical Phenomena. I. Renormalization Group and the Kadanoff Scaling Picture"
    Kadanoff (1966) "Scaling Laws for Ising Models near Tc"
    Li et al. (2018) "Neural Network Renormalization Group"

Question:
    Does the circuit's description simplify under coarse-graining
    (renormalization)? Is the computation scale-invariant — does it
    look the same at different levels of granularity?

Metrics:
    renormalization — Fixed points, scale invariance, and effective dimensionality under RG flow
...
Usage:
    ...
    from protocols.cross_discipline.renormalization import run_protocol
...
PROTOCOL_ID = "PH_RG"
PROTOCOL_NAME = "Renormalization Group"
METRICS = ["renormalization"]
CALIBRATIONS = STRUCTURAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "ph_renormalization"
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all PH_RG metrics + calibrations. Returns a ProtocolResult."""
...
def rg_analysis(result: ProtocolResult) -> list[str]:
    """Analyze results through renormalization group lens."""
    lines = ["\n  Renormalization Group Analysis:", "  --------------------------------"]

    for task in result.tasks:
        lines.append(f"\n    {task}:")

        rg = _find(result.metrics.get("renormalization", []), task)

        properties = []

        if rg:
            lines.append(f"      Renormalization = {rg.value:.4f}")
            if rg.value > THRESHOLDS["renormalization"]:
                properties.append("scale-invariant")
                lines.append(f"        Circuit simplifies under coarse-graining — fixed point found")
            elif rg.value > 0.2:
                properties.append("approximately-invariant")
                lines.append(f"        Approximate scale invariance — near a fixed point")
            else:
                lines.append(f"        No scale invariance — circuit is scale-dependent")

            # Check for additional RG metadata if available
            fp = rg.metadata.get("fixed_point_distance", None)
            if fp is not None:
                lines.append(f"      Fixed point distance = {fp:.4f}")

            eff_dim = rg.metadata.get("effective_dimensionality", None)
            if eff_dim is not None:
                lines.append(f"      Effective dimensionality = {eff_dim:.4f}")
                if eff_dim < 3.0:
                    properties.append("low-dimensional")

        if properties:
            verdict = f"RG properties: {', '.join(properties)}"
        else:
            verdict = "No renormalization group structure detected"
...
def summarize(result: ProtocolResult) -> str:
    ...
    lines.extend(rg_analysis(result))
