"""Protocol PH_CP — Critical Phenomena
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Dynamical Systems
Validity Type: Construct
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/cross_discipline/critical-phenomena/
Family:       Cross-discipline (Physics)
Validity:     External — statistical physics paradigms applied to circuit behavior

References:
    Wilson & Kogut (1974) "The renormalization group and the epsilon expansion"
    Anderson (1972) "More Is Different"
    Goldenfeld (1992) "Lectures on Phase Transitions and the Renormalization Group"

Question:
    Does the circuit exhibit phase transitions — abrupt behavioral
    changes as intervention strength is scaled? Does it show symmetry
    breaking where equivalent representations become asymmetric?

Metrics:
    phase_transitions — Abrupt behavioral changes under scaled interventions
    symmetry_breaking — Equivalent representations becoming asymmetric
...
Usage:
    ...
    from protocols.cross_discipline.critical_phenomena import run_protocol
...
PROTOCOL_ID = "PH_CP"
PROTOCOL_NAME = "Critical Phenomena"
METRICS = ["phase_transitions", "symmetry_breaking"]
CALIBRATIONS = STRUCTURAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "ph_critical_phenomena"
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all PH_CP metrics + calibrations. Returns a ProtocolResult."""
...
def critical_analysis(result: ProtocolResult) -> list[str]:
    """Analyze results through critical phenomena lens."""
    lines = ["\n  Critical Phenomena Analysis:", "  -----------------------------"]

    for task in result.tasks:
        lines.append(f"\n    {task}:")

        pt = _find(result.metrics.get("phase_transitions", []), task)
        sb = _find(result.metrics.get("symmetry_breaking", []), task)

        properties = []

        if pt:
            lines.append(f"      Phase transitions = {pt.value:.4f}")
            if pt.value > THRESHOLDS["phase_transitions"]:
                properties.append("phase-transition")
                lines.append(f"        Abrupt transition detected — critical point exists")
            else:
                lines.append(f"        Smooth degradation — no sharp phase boundary")

        if sb:
            lines.append(f"      Symmetry breaking = {sb.value:.4f}")
            if sb.value > 0.5:
                properties.append("broken-symmetry")
                lines.append(f"        Strong symmetry breaking — representations are asymmetric")
            elif sb.value > 0.1:
                properties.append("weak-asymmetry")
                lines.append(f"        Weak symmetry breaking — slight representational asymmetry")
            else:
                lines.append(f"        Symmetric — equivalent representations remain equivalent")

        if properties:
            verdict = f"Critical properties: {', '.join(properties)}"
        else:
            verdict = "No critical phenomena detected"
...
def summarize(result: ProtocolResult) -> str:
    ...
    lines.extend(critical_analysis(result))
