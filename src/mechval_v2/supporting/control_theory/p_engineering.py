"""Protocol EN — Engineering Robustness
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Control Theory
Validity Type: Internal
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/cross_discipline/engineering/
Family:       Cross-discipline (Engineering)
Validity:     External — robustness and fault tolerance of circuit components

References:
    Leveson (2011) "Engineering a Safer World" — systems safety
    Avizienis et al. (2004) "Basic Concepts and Taxonomy of Dependable Computing"
    Strogatz (2001) "Exploring Complex Networks" — robustness in networks

Question:
    How robust is the circuit under stress? Graceful degradation tests
    whether performance declines smoothly as components are removed (vs
    catastrophic failure). Fault tolerance measures recovery from component
    perturbation. Stress testing pushes the circuit beyond normal operating
    conditions. Error propagation tracks how local faults cascade.

Metrics:
    graceful_degradation — Smooth vs catastrophic performance decline
    fault_tolerance      — Recovery from component perturbation
    stress_testing       — Performance under extreme conditions
    error_propagation    — Fault cascade analysis
...
Usage:
    ...
    from protocols.cross_discipline.engineering import run_protocol
...
PROTOCOL_ID = "EN"
PROTOCOL_NAME = "Engineering Robustness"
METRICS = ["graceful_degradation", "fault_tolerance", "stress_testing", "error_propagation"]
CALIBRATIONS = STRUCTURAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "en_engineering"
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all EN metrics + calibrations. Returns a ProtocolResult."""
...
def engineering_analysis(result: ProtocolResult) -> list[str]:
    """Analyze results through engineering robustness lens."""
    lines = ["\n  Engineering Robustness Analysis:", "  ---------------------------------"]

    for task in result.tasks:
        lines.append(f"\n    {task}:")

        gd = _find(result.metrics.get("graceful_degradation", []), task)
        ft = _find(result.metrics.get("fault_tolerance", []), task)
        st = _find(result.metrics.get("stress_testing", []), task)
        ep = _find(result.metrics.get("error_propagation", []), task)

        properties = []

        if gd:
            lines.append(f"      Graceful degradation = {gd.value:.4f}")
            if gd.value > THRESHOLDS["graceful_degradation"]:
                properties.append("graceful")
            else:
                lines.append(f"        Warning: catastrophic failure mode detected")

        if ft:
            lines.append(f"      Fault tolerance = {ft.value:.4f}")
            if ft.value > THRESHOLDS["fault_tolerance"]:
                properties.append("fault-tolerant")

        if st:
            lines.append(f"      Stress testing = {st.value:.4f}")
            if st.value > THRESHOLDS["stress_testing"]:
                properties.append("stress-resistant")

        if ep:
            lines.append(f"      Error propagation = {ep.value:.4f}")
            if ep.value < 0.3:
                properties.append("fault-contained")
            else:
                lines.append(f"        Warning: errors propagate through circuit")

        if properties:
            verdict = f"Robustness properties: {', '.join(properties)}"
        else:
            verdict = "Circuit is fragile — no robustness properties detected"
...
def summarize(result: ProtocolResult) -> str:
    ...
    lines.extend(engineering_analysis(result))
