"""Protocol ECON — Mechanism Design
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Economics
Validity Type: External
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/cross_discipline/economics/
Family:       Cross-discipline (Economics)
Validity:     External — mechanism design analysis of attention allocation
...
Question:
    Does the circuit's attention allocation behave like an optimal auction
    mechanism? Do heads compete for attention in ways that satisfy mechanism
    design properties (incentive compatibility, individual rationality)? Is
    attention allocation efficient?

Metrics:
    mechanism_design  — Incentive compatibility and mechanism efficiency
    attention_auction — Auction-theoretic analysis of attention allocation

Usage:
    uv run python economics.py
    uv run python economics.py --tasks ioi induction --device cuda

    from protocols.cross_discipline.economics import run_protocol
...
PROTOCOL_ID = "ECON"
PROTOCOL_NAME = "Mechanism Design"
METRICS = ["mechanism_design", "attention_auction"]
CALIBRATIONS = STRUCTURAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "econ_mechanism_design"

THRESHOLDS = {
    "mechanism_design": 0.5,
    "attention_auction": 0.5,
}
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all ECON metrics + calibrations. Returns a ProtocolResult."""
...
def mechanism_analysis(result: ProtocolResult) -> list[str]:
    ...
    for task in result.tasks:
        ...
        md = _find(result.metrics.get("mechanism_design", []), task)
        aa = _find(result.metrics.get("attention_auction", []), task)

        properties = []

        if md:
            lines.append(f"      Mechanism design = {md.value:.4f}")
            if md.value > THRESHOLDS["mechanism_design"]:
                lines.append(f"        Incentive compatible — truthful reporting is optimal")
                properties.append("incentive-compatible")
            else:
                lines.append(f"        Not incentive compatible — heads can benefit from misreporting")

        if aa:
            lines.append(f"      Attention auction = {aa.value:.4f}")
            if aa.value > THRESHOLDS["attention_auction"]:
                lines.append(f"        Efficient allocation — resembles second-price auction")
                properties.append("efficient-allocation")
            else:
                lines.append(f"        Inefficient allocation — attention not optimally distributed")

        if md and aa:
            if md.value > THRESHOLDS["mechanism_design"] and aa.value > THRESHOLDS["attention_auction"]:
                lines.append(f"        Revenue equivalence holds — mechanism is near-optimal")
                properties.append("revenue-equivalent")

        if properties:
            verdict = f"Mechanism properties: {', '.join(properties)}"
        else:
            verdict = "No mechanism design properties detected"
