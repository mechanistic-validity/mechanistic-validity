"""Protocol A08 — Partial Information Decomposition
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Information Theory
Validity Type: Internal
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/causal/a08-pid/
Family:       Causal (Information-Theoretic Decomposition)
...
Question:
    How is the information about the task output decomposed across circuit
    components? What fraction is redundant (shared by multiple heads),
    unique (carried by a single component), or synergistic (only available
    when components are considered jointly)?

    High synergy indicates composition — the circuit's value exceeds the
    sum of its parts. High redundancy suggests over-specification or
    robustness. High unique information localizes function.

Metrics:
    pid — Partial Information Decomposition: redundancy, unique, synergy
          breakdown across circuit heads for each task
...
Usage:
    uv run python a08_pid.py                       # all tasks, CPU
    uv run python a08_pid.py --device cuda          # GPU
    uv run python a08_pid.py --tasks ioi induction  # specific tasks

    # As a callable module:
    from protocols.causal.a08_pid import run_protocol
...
PROTOCOL_ID = "A08"
PROTOCOL_NAME = "Partial Information Decomposition"
METRICS = ["pid"]
CALIBRATIONS = CAUSAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "a08_pid"
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all A08 metrics + calibrations. Returns a ProtocolResult."""
...
def decomposition_analysis(result: ProtocolResult) -> list[str]:
    """Analyze PID decomposition: redundancy, unique, synergy."""
    lines = ["\n  Information Decomposition Analysis:", "  ────────────────────────────────────"]

    pid_results = result.metrics.get("pid", [])

    for task in result.tasks:
        lines.append(f"\n    {task}:")

        match = _find(pid_results, task)
        if not match:
            lines.append(f"      No PID data available")
            continue

        lines.append(f"      PID score: {match.value:+.4f}")

        redundancy = match.metadata.get("redundancy", None)
        unique = match.metadata.get("unique", None)
        synergy = match.metadata.get("synergy", None)
        ...
        if synergy is not None and redundancy is not None:
            if synergy > redundancy:
                lines.append(f"      -> Synergy-dominated: circuit components compose non-trivially")
            elif redundancy > synergy:
                lines.append(f"      -> Redundancy-dominated: robust but potentially over-specified")
            else:
                lines.append(f"      -> Balanced decomposition")
