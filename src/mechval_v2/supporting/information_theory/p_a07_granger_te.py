"""Protocol A07 — Granger Causality & Transfer Entropy
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Information Theory
Validity Type: Internal
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/causal/a07-granger-te/
Family:       Causal (Temporal and Information-Theoretic)
Validity:     Internal — I1 Necessity
...
Question:
    Do circuit components carry directed information flow consistent with
    the hypothesized causal structure? Does knowledge of upstream circuit
    activations improve prediction of downstream behavior beyond what the
    past of the target alone provides?

Metrics:
    cross_task_transfer  — Cross-task IIA transfer: does a circuit learned
                           for one task generalize causally to another?
    ocse                 — Observational Causal Structure Estimation
    transfer_entropy     — Directed information flow between components
    granger_causality    — Temporal precedence in activation sequences
...
Usage:
    uv run python a07_granger_te.py                       # all tasks, CPU
    uv run python a07_granger_te.py --device cuda          # GPU
    uv run python a07_granger_te.py --tasks ioi induction  # specific tasks

    # As a callable module:
    from protocols.causal.a07_granger_te import run_protocol
...
PROTOCOL_ID = "A07"
PROTOCOL_NAME = "Granger Causality & Transfer Entropy"
METRICS = ["cross_task_transfer", "ocse", "transfer_entropy", "granger_causality"]
CALIBRATIONS = CAUSAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "a07_granger_te"

THRESHOLDS = {
    "cross_task_transfer": 0.3,
    "ocse": 0.5,
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all A07 metrics + calibrations. Returns a ProtocolResult."""
...
def information_flow_analysis(result: ProtocolResult) -> list[str]:
    """Analyze results through directed information flow lens."""
    ...
    for task in result.tasks:
        ...
        oc = _find(result.metrics.get("ocse", []), task)

        evidence = []

        if te:
            lines.append(f"      Transfer entropy: {te.value:+.4f}")
            if te.value > THRESHOLDS["transfer_entropy"]:
                evidence.append("directed info flow")

        if gc:
            lines.append(f"      Granger causality: {gc.value:+.4f}")
            if gc.value > THRESHOLDS["granger_causality"]:
                evidence.append("temporal precedence")

        if ct:
            lines.append(f"      Cross-task transfer: {ct.value:+.4f}")
            if ct.value > THRESHOLDS["cross_task_transfer"]:
                evidence.append("cross-task generalization")

        if oc:
            lines.append(f"      OCSE: {oc.value:+.4f}")
            if oc.value > THRESHOLDS["ocse"]:
                evidence.append("observational structure")

        n = len(evidence)
        verdict = (
            f"Strong causal flow evidence ({', '.join(evidence)})" if n >= 3
            else f"Partial evidence ({', '.join(evidence)})" if n >= 1
            else "No directed causal flow evidence"
        )
