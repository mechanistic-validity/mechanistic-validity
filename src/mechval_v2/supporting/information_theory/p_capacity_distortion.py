"""Protocol IT_CD — Capacity and Distortion
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Information Theory
Validity Type: Measurement
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/cross_discipline/capacity-distortion/
Family:       Cross-discipline (Information Theory)
Validity:     External — information-theoretic capacity and rate-distortion analysis
...
Question:
    What is the information-theoretic capacity of the circuit's communication
    channels? What is the tradeoff between compression and fidelity — how
    much can the circuit compress representations while preserving
...
Usage:
    uv run python capacity_distortion.py
    uv run python capacity_distortion.py --tasks ioi induction --device cuda

    from protocols.cross_discipline.capacity_distortion import run_protocol
...
PROTOCOL_ID = "IT_CD"
PROTOCOL_NAME = "Capacity and Distortion"
METRICS = ["channel_capacity", "rate_distortion"]
CALIBRATIONS = STRUCTURAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "it_cd_capacity_distortion"
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all IT_CD metrics + calibrations. Returns a ProtocolResult."""
...
def capacity_analysis(result: ProtocolResult) -> list[str]:
    """Protocol-specific interpretive analysis."""
    lines = ["\n  Capacity and Distortion Analysis:", "  -----------------------------------"]
    ...
    for task in result.tasks:
        ...
        if cc:
            ...
            if cc.value > THRESHOLDS["channel_capacity"]:
                lines.append(f"        Operating near theoretical maximum")
                properties.append("high-capacity")
            else:
                lines.append(f"        Below theoretical capacity — channel underutilized")
                properties.append("low-capacity")

        if rd:
            lines.append(f"      Rate-distortion = {rd.value:.4f}")
            if rd.value > THRESHOLDS["rate_distortion"]:
                lines.append(f"        Efficient compression — near Shannon limit")
                properties.append("efficient-compression")
            else:
                lines.append(f"        Lossy compression — significant distortion")
                properties.append("lossy")

        if cc and rd:
            if cc.value > THRESHOLDS["channel_capacity"] and rd.value > THRESHOLDS["rate_distortion"]:
                lines.append(f"        Circuit operates near Shannon limit with efficient coding")

        if properties:
            verdict = f"Information-theoretic properties: {', '.join(properties)}"
        else:
            verdict = "No information-theoretic properties detected"
...
def summarize(result: ProtocolResult) -> str:
    ...
    lines.extend(capacity_analysis(result))
