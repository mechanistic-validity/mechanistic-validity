"""Protocol GN_EV — Evolutionary Analysis
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Genetics
Validity Type: External
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/cross_discipline/evolutionary-analysis/
Family:       Cross-discipline (Genetics)
Validity:     External — evolutionary biology paradigms applied to circuit development

References:
    Conway Morris (2003) "Life's Solution: Inevitable Humans in a Lonely Universe"
    Felsenstein (2004) "Inferring Phylogenies"

Question:
    Do different model architectures converge on similar circuit
    structures (convergent evolution)? How do circuits change across
    training checkpoints (phylogenetic tracking)?

Metrics:
    convergent_evolution  — Do independent models converge on similar circuit structures?
    phylogenetic_tracking — How do circuits change across training checkpoints?

Usage:
    uv run python evolutionary_analysis.py                       # all tasks, CPU
    uv run python evolutionary_analysis.py --device cuda          # GPU
    uv run python evolutionary_analysis.py --tasks ioi induction  # specific tasks

    # As a callable module:
    from protocols.cross_discipline.evolutionary_analysis import run_protocol
...
PROTOCOL_ID = "GN_EV"
PROTOCOL_NAME = "Evolutionary Analysis"
METRICS = ["convergent_evolution", "phylogenetic_tracking"]
CALIBRATIONS = STRUCTURAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "gn_evolutionary_analysis"

THRESHOLDS = {
    "convergent_evolution": 0.5,
    "phylogenetic_tracking": 0.0,
}
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all GN_EV metrics + calibrations. Returns a ProtocolResult."""
...
def evolutionary_analysis(result: ProtocolResult) -> list[str]:
    """Analyze results through evolutionary biology lens."""
    lines = ["\n  Evolutionary Analysis:", "  -----------------------"]

    for task in result.tasks:
        lines.append(f"\n    {task}:")

        ce = _find(result.metrics.get("convergent_evolution", []), task)
        pt = _find(result.metrics.get("phylogenetic_tracking", []), task)

        properties = []

        if ce:
            lines.append(f"      Convergent evolution = {ce.value:.4f}")
            if ce.value > THRESHOLDS["convergent_evolution"]:
                properties.append("convergent")
                lines.append(f"        Independent models converge on similar circuits")
            else:
                lines.append(f"        Circuits diverge across architectures")

        if pt:
            lines.append(f"      Phylogenetic tracking = {pt.value:.4f}")
            if pt.value > 0.5:
                properties.append("gradual-evolution")
                lines.append(f"        Circuit evolves gradually during training")
            elif pt.value > 0.0:
                properties.append("punctuated-evolution")
                lines.append(f"        Circuit shows punctuated equilibrium — sudden changes")
            else:
                properties.append("static")
                lines.append(f"        Circuit structure stable across training")

        if properties:
            verdict = f"Evolutionary properties: {', '.join(properties)}"
        else:
            verdict = "No evolutionary patterns detected"
...
def summarize(result: ProtocolResult) -> str:
    ...
    lines.extend(evolutionary_analysis(result))
