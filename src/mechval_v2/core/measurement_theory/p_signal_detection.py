"""Protocol EX01 — Signal Detection Theory
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Lens:         Measurement Theory
Validity Type: Measurement
Framework:    https://mechanistic-validity.github.io/mechanistic-validity/framework/metrics/cognitive_science/signal-detection/
Family:       Cognitive Science (Signal Detection)
Validity:     External — discriminability of circuit-mediated representations

References:
    Green & Swets (1966) "Signal Detection Theory and Psychophysics"
    Macmillan & Creelman (2004) "Detection Theory: A User's Guide"

Question:
    Can circuit components discriminate between signal (task-relevant input)
    and noise (task-irrelevant input) as measured by d-prime? High d' means
    circuit representations cleanly separate signal from noise.

Metrics:
    dprime — Signal detection d' for circuit head activations

Usage:
    uv run python signal_detection.py                       # all tasks, CPU
    uv run python signal_detection.py --device cuda          # GPU
    uv run python signal_detection.py --tasks ioi induction  # specific tasks

    # As a callable module:
    from protocols.cognitive_science.signal_detection import run_protocol
...
PROTOCOL_ID = "EX01"
PROTOCOL_NAME = "Signal Detection Theory"
METRICS = ["dprime"]
CALIBRATIONS = BEHAVIORAL_CALIBRATIONS
OUTPUT_DIR = Path(__file__).parent / "results" / "ex01_signal_detection"
...
def run_protocol(model, tasks: list[str] | None = None, n_prompts: int = 40,
    ...
    """Run all EX01 metrics + calibrations. Returns a ProtocolResult."""
    ...
    for metric_name in METRICS:
        ...
        for r in results:
            ...
            print(f"    {task:20s}  d'={r.value:+.4f}{tag}")
...
def sdt_analysis(result: ProtocolResult) -> list[str]:
    """Analyze results through signal detection theory lens."""
    lines = ["\n  Signal Detection Analysis:", "  --------------------------"]

    for task in result.tasks:
        dp = _find(result.metrics.get("dprime", []), task)
        if dp is None:
            continue
        ...
        if d > 2.0:
            ...
            quality = "Weak discrimination"
        else:
            quality = "Near chance — no discrimination"

        lines.append(f"\n    {task}:")
        lines.append(f"      d' = {d:.4f} — {quality}")
...
def summarize(result: ProtocolResult) -> str:
    ...
    lines.extend(sdt_analysis(result))
