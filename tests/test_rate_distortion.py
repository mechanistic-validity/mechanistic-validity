...
_MOD_PATH = (
    ...
    / "src" / "mechval" / "metrics" / "extended" / "information_theory" / "IT2_rate_distortion.py"
)
...
run_rate_distortion = _mod.run_rate_distortion
...
def results(gpt2_model):
    return run_rate_distortion(gpt2_model, [TASK], n_prompts=5)
...
def test_metric_id(results):
    assert results[0].metric_id == "IT2.rate_distortion"
