...
_MOD_PATH = (
    ...
    / "src" / "mechval" / "metrics" / "extended" / "economics" / "ECON2_attention_auction.py"
)
...
run_attention_auction = _mod.run_attention_auction
...
def results(gpt2_model):
    return run_attention_auction(gpt2_model, [TASK], n_prompts=5)
...
def test_metric_id(results):
    assert results[0].metric_id == "ECON2.attention_auction"
