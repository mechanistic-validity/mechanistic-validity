...
def circuit_results(gpt2_model):
    return run_dose_response(
        gpt2_model, tasks=[TASK], n_prompts=5, n_random_baselines=3,
    )


def test_run_dose_response_returns_eval_results(circuit_results):
    assert len(circuit_results) == 1
    assert isinstance(circuit_results[0], EvalResult)


def test_run_dose_response_metric_id(circuit_results):
    assert circuit_results[0].metric_id == "B95.dose_response"


def test_run_dose_response_metadata_keys(circuit_results):
...
def test_run_dose_response_monotonicity_in_range(circuit_results):
    mono = circuit_results[0].metadata["monotonicity"]
    assert 0.0 <= mono <= 1.0


def test_run_dose_response_selectivity_positive(circuit_results):
    sel = circuit_results[0].metadata["selectivity"]
    assert sel > 0.0


def test_run_dose_response_curve_length_matches_fractions(circuit_results):
    curve = circuit_results[0].metadata["circuit_curve"]
    fracs = circuit_results[0].metadata["fractions"]
    assert len(curve) == len(fracs)
    assert len(fracs) == 5
    assert fracs == ABLATION_FRACTIONS


def test_run_dose_response_value_equals_monotonicity(circuit_results):
    assert circuit_results[0].value == pytest.approx(circuit_results[0].metadata["monotonicity"])


def test_run_dose_response_n_samples_positive(circuit_results):
    assert circuit_results[0].n_samples > 0
    assert circuit_results[0].n_samples <= 5


def test_run_dose_response_circuit_heads_positive(circuit_results):
    assert circuit_results[0].metadata["n_circuit_heads"] > 0


def test_run_dose_response_baseline_random_set(circuit_results):
    assert circuit_results[0].baseline_random is not None


def test_run_dose_response_clean_curve_first_point_nonzero(circuit_results):
    curve = circuit_results[0].metadata["circuit_curve"]
